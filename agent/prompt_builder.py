...
DEFAULT_AGENT_IDENTITY = (
    "You are Iris Agent, an intelligent AI assistant created by Metaxis Research. "
...
def _read_skill_conditions(skill_file: Path) -> dict:
    ...
    try:
        ...
        raw = skill_file.read_text(encoding="utf-8")[:2000]
        frontmatter, _ = _parse_frontmatter(raw)
        Iris = frontmatter.get("metadata", {}).get("Iris", {})
        return {
            "fallback_for_toolsets": Iris.get("fallback_for_toolsets", []),
            "requires_toolsets": Iris.get("requires_toolsets", []),
            "fallback_for_tools": Iris.get("fallback_for_tools", []),
            "requires_tools": Iris.get("requires_tools", []),
        }
    except Exception:
        return {}
...
def build_skills_system_prompt(
    ...
    Scans ~/.Iris/skills/ for SKILL.md files grouped by category.
...
def build_context_files_prompt(cwd: Optional[str] = None) -> str:
    ...
    SOUL.md (cwd then ~/.Iris/ fallback). Each capped at 20,000 chars.
    """
    if cwd is None:
        cwd = os.getcwd()
    ...
    # SOUL.md (cwd first, then ~/.Iris/ fallback)
    soul_path = None
    for name in ["SOUL.md", "soul.md"]:
        ...
        if candidate.exists():
            ...
            break
    if not soul_path:
        global_soul = Path.home() / ".Iris" / "SOUL.md"
