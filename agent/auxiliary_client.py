...
Resolution order for text tasks (auto mode):
  ...
  2. Metaxis Portal (~/.Iris/auth.json active provider)
...
"openrouter", "Metaxis", "codex", or "main" (= steps 3-5).
...
_OR_HEADERS = {
    "HTTP-Referer": "https://github.com/Metaxis Research/iris-agent",
    "X-OpenRouter-Title": "Iris Agent",
    "X-OpenRouter-Categories": "productivity,cli-agent",
}

# Metaxis Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Metaxis Portal.
Metaxis_EXTRA_BODY = {"tags": ["product=iris-agent"]}

# Set at resolve time — True if the auxiliary client points to Metaxis Portal
auxiliary_is_Metaxis: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_Metaxis_MODEL = "gemini-3-flash"
_Metaxis_DEFAULT_BASE_URL = "https://inference-api.Metaxis Research.com/v1"
_AUTH_JSON_PATH = Path.home() / ".Iris" / "auth.json"
...
def _read_Metaxis_auth() -> Optional[dict]:
    """Read and validate ~/.Iris/auth.json for an active Metaxis provider.

    Returns the provider state dict if Metaxis is active with tokens,
    otherwise None.
    """
    try:
        ...
        if data.get("active_provider") != "Metaxis":
            return None
        provider = data.get("providers", {}).get("Metaxis", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Metaxis auth: %s", exc)
        return None


def _Metaxis_api_key(provider: dict) -> str:
    """Extract the best API key from a Metaxis provider state dict."""
    return provider.get("agent_key") or provider.get("access_token", "")


def _Metaxis_base_url() -> str:
    """Resolve the Metaxis inference base URL from env or default."""
    return os.getenv("Metaxis_INFERENCE_BASE_URL", _Metaxis_DEFAULT_BASE_URL)


def _read_codex_access_token() -> Optional[str]:
    """Read a valid Codex OAuth access token from Iris auth store (~/.Iris/auth.json)."""
    try:
        from Iris_cli.auth import _read_codex_tokens
...
def _get_auxiliary_provider(task: str = "") -> str:
    ...
    then falls back to "auto".  Returns one of: "auto", "openrouter", "Metaxis", "main".
    """
    if task:
...
def _try_Metaxis() -> Tuple[Optional[OpenAI], Optional[str]]:
    Metaxis = _read_Metaxis_auth()
    if not Metaxis:
        return None, None
    global auxiliary_is_Metaxis
    auxiliary_is_Metaxis = True
    logger.debug("Auxiliary client: Metaxis Portal")
    return (
        OpenAI(api_key=_Metaxis_api_key(Metaxis), base_url=_Metaxis_base_url()),
        _Metaxis_MODEL,
    )
...
def _resolve_forced_provider(forced: str) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if forced == "Metaxis":
        client, model = _try_Metaxis()
        if client is None:
            logger.warning("auxiliary.provider=Metaxis but Metaxis Portal not configured (run: Iris login)")
        return client, model

    if forced == "codex":
        client, model = _try_codex()
        if client is None:
            logger.warning("auxiliary.provider=codex but no Codex OAuth token found (run: Iris model)")
        return client, model

    if forced == "main":
        # "main" = skip OpenRouter/Metaxis, use the main chat model's credentials.
...
def _resolve_auto() -> Tuple[Optional[OpenAI], Optional[str]]:
    """Full auto-detection chain: OpenRouter → Metaxis → custom → Codex → API-key → None."""
    for try_fn in (_try_openrouter, _try_Metaxis, _try_custom_endpoint,
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "Metaxis", "openai-codex" (or "codex"),
    ...
    # ── Metaxis Portal (OAuth) ──────────────────────────────────────────
    if provider == "Metaxis":
        client, default = _try_Metaxis()
        if client is None:
            logger.warning("resolve_provider_client: Metaxis requested "
                           "but Metaxis Portal not configured (run: Iris login)")
            return None, None
        final_model = model or default
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: Iris model)")
                return None, None
    ...
    try:
        from Iris_cli.auth import PROVIDER_REGISTRY, _resolve_kimi_base_url
    except ImportError:
        logger.debug("Iris_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "Metaxis":
            return resolve_provider_client("Metaxis", model, async_mode)
        if provider == "openai-codex":
...
def get_vision_auxiliary_client() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    OpenRouter, Metaxis Portal, and Codex OAuth (gpt-5.3-codex supports
    ...
    for try_fn in (_try_openrouter, _try_Metaxis, _try_codex,
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Metaxis Portal product tags when the auxiliary client is backed
    by Metaxis Portal. Returns empty dict otherwise.
    """
    return dict(Metaxis_EXTRA_BODY) if auxiliary_is_Metaxis else {}
...
def auxiliary_max_tokens_param(value: int) -> dict:
    ...
    if (not or_key
            and _read_Metaxis_auth() is None
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        # Codex adapter handles max_tokens internally; OpenRouter/Metaxis use max_tokens.
    ...
    if provider == "Metaxis" or auxiliary_is_Metaxis:
        merged_extra.setdefault("tags", []).extend(["product=iris-agent"])
    if merged_extra:
...
def call_llm(
    ...
    *,
    ...
    """Centralized synchroMetaxis LLM call.
...
async def async_call_llm(
    ...
    *,
    ...
    """Centralized asynchroMetaxis LLM call.
