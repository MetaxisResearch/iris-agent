#!/usr/bin/env python3
"""
Iris Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Iris Agent, inspired by Claude Code.
...
os.environ["Iris_QUIET"] = "1"  # Our own modules
...
_Iris_home = Path(os.getenv("Iris_HOME", Path.home() / ".Iris"))
_user_env = _Iris_home / ".env"
...
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(_Iris_home))
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.Iris/.
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.Iris/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.
    """
    # Check user config first (~/.Iris/config.yaml)
    user_config_path = Path.home() / '.Iris' / 'config.yaml'
    ...
    # Normalize config key: the new config system (Iris_cli/config.py) and all
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["Iris_REDACT_SECRETS"] = str(redact).lower()
...
try:
    from Iris_cli.skin_engine import init_skin_from_config
...
from Iris_cli.banner import (
    cprint as _cprint, _GOLD, _BOLD, _DIM, _RST,
    VERSION, RELEASE_DATE, Iris_AGENT_LOGO, Iris_CADUCEUS, COMPACT_BANNER,
    get_available_skills as _get_available_skills,
    build_welcome_banner,
)
from Iris_cli.commands import COMMANDS, SlashCommandCompleter
from Iris_cli import callbacks as _callbacks
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run Iris -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"Iris-{short_id}"
    branch_name = f"Iris/{wt_name}"
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("Iris-"):
            continue
...
# ASCII Art - iris-agent logo (full width, single line - requires ~95 char terminal)
Iris_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Iris Caduceus (compact, fits in left panel)
Iris_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
COMPACT_BANNER = """
[bold #FFD700]╔══════════════════════════════════════════════════════════════╗[/]
[bold #FFD700]║[/]  [#FFBF00]⚕ Metaxis Iris[/] [dim #B8860B]- AI Agent Framework[/]              [bold #FFD700]║[/]
[bold #FFD700]║[/]  [#CD7F32]Messenger of the Digital Gods[/]    [dim #B8860B]Metaxis Research[/]   [bold #FFD700]║[/]
[bold #FFD700]╚══════════════════════════════════════════════════════════════╝[/]
"""
...
def _build_compact_banner() -> str:
    ...
    if w < 30:
        return "\n[#FFBF00]⚕ Metaxis Iris[/] [dim #B8860B]- Metaxis Research[/]\n"
    inner = w - 2  # inside the box border
    bar = "═" * w
    line1 = "⚕ Metaxis Iris - AI Agent Framework"
    line2 = "Messenger of the Digital Gods  ·  Metaxis Research"
...
def _get_available_skills() -> Dict[str, List[str]]:
    """
    Scan ~/.Iris/skills/ and return skills grouped by category.
    ...
    Iris_home = Path(os.getenv("Iris_HOME", Path.home() / ".Iris"))
    skills_dir = Iris_home / "skills"
    skills_by_category = {}
...
def build_welcome_banner(console: Console, model: str, cwd: str, tools: List[dict] = None, enabled_toolsets: List[str] = None, session_id: str = None, context_length: int = None):
    ...
    try:
        from Iris_cli.skin_engine import get_active_skin
        ...
        _agent_name = _bskin.get_branding("agent_name", "Iris Agent")
    except Exception:
        _bskin = None
        _accent, _dim, _text = "#FFBF00", "#B8860B", "#FFF8DC"
        _session_c, _title_c, _border_c = "#8B8682", "#FFD700", "#CD7F32"
        _agent_name = "Iris Agent"

    _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else Iris_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{_accent}]{model_short}[/]{ctx_str} [dim {_dim}]·[/] [dim {_dim}]Metaxis Research[/]")
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else Iris_AGENT_LOGO
        console.print(_logo)
        console.print()
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.Iris/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.Iris/config.yaml on first use)
...
class IrisCLI:
    """
    Interactive CLI for the Iris Agent.
    ...
    def __init__(
        ...
        """
        Initialize the Iris CLI.

        Args:
            model: Model to use (default: from env or claude-sonnet)
            toolsets: List of toolsets to enable (default: all)
            provider: Inference provider ("auto", "openrouter", "Metaxis", "openai-codex", "zai", "kimi-coding", "minimax", "minimax-cn")
        ...
        self.requested_provider = (
            provider
            or os.getenv("Iris_INFERENCE_PROVIDER")
        ...
        self._Metaxis_key_expires_at: Optional[str] = None
        self._Metaxis_key_source: Optional[str] = None
        ...
        elif os.getenv("Iris_MAX_ITERATIONS"):
            self.max_turns = int(os.getenv("Iris_MAX_ITERATIONS"))
        else:
            self.max_turns = 90
        ...
        self.system_prompt = (
            os.getenv("Iris_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        self._history_file = Path.home() / ".Iris_history"
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from Iris_cli.runtime_provider import (
    ...
    def _init_agent(self) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (Iris sessions list).{_RST}")
                return False
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Iris: ", style="dim bold #8FBC8F")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.Iris/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from Iris_cli.clipboard import save_clipboard_image

        img_dir = Path.home() / ".Iris" / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: Iris --checkpoints")
    ...
    def _handle_paste_command(self):
        ...
        """
        from Iris_cli.clipboard import has_clipboard_image
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self.console.print("[dim]   Run 'Iris setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from Iris_cli.commands import COMMANDS_BY_CATEGORY
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Iris!{_RST}")
    ...
    def _show_model_and_providers(self):
        ...
        from Iris_cli.auth import resolve_provider as _resolve_provider
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to Iris_cli.skills_hub."""
        from Iris_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print("  Configuration file: ~/.Iris/gateway.json")
            print()
            ...
            print("    2. Or create ~/.Iris/gateway.json")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        if cmd_lower in ("/quit", "/exit", "/q"):
            ...
            parts = cmd_original.split(maxsplit=1)
            if len(parts) > 1:
                from Iris_cli.auth import resolve_provider
                from Iris_cli.models import (
                    parse_model_input,
                ...
                if provider_changed:
                    try:
                        ...
                        if target_provider == "custom":
                            ...
                            print(f"      or run: Iris setup → Custom OpenAI-compatible endpoint")
                        else:
            ...
            try:
                ...
                if response:
                    try:
                        ...
                        label = _skin.get_branding("response_label", "⚕ Iris")
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from Iris_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print(f"\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in ~/.Iris/skins/\n")
            return
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            for quiet_logger in ('tools', 'minisweagent', 'run_agent', 'trajectory_compressor', 'cron', 'Iris_cli'):
                    ...
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                ...
                                _dbg = _pl.Path.home() / ".Iris" / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  Iris --resume {self.session_id}")
            print()
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Iris Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Iris Agent! Type your message or /help for commands."
        ...
        # Dynamic prompt: shows Iris symbol when agent is working,
        ...
        def _on_text_changed(buf):
            ...
            if line_count >= 5 and chars_added > 1 and not text.startswith('/'):
                _paste_counter[0] += 1
                # Save to temp file
                paste_dir = Path(os.path.expanduser("~/.Iris/pastes"))
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Iris needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)

            lines = []
            # Box top border
            lines.append(('class:clarify-border', '╭─ '))
            lines.append(('class:clarify-title', 'Iris needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Iris needs your input") - 3)) + '╮\n'))
...
def main(
    ...
    """
    Iris Agent CLI - Interactive AI Assistant
    ...
    os.environ["Iris_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Iris Gateway (messaging platforms)...")
    ...
    # Default to Iris-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Check config for CLI toolsets, fallback to Iris-cli
