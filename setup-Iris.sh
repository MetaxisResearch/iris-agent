...
# 5. Symlinks the 'Iris' CLI command into ~/.local/bin
...
echo ""
echo -e "${CYAN}⚕ Iris Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink Iris into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up Iris command..."

Iris_BIN="$SCRIPT_DIR/venv/bin/Iris"
mkdir -p "$HOME/.local/bin"
ln -sf "$Iris_BIN" "$HOME/.local/bin/Iris"
echo -e "${GREEN}✓${NC} Symlinked Iris → ~/.local/bin/Iris"
...
# ============================================================================
# Seed bundled skills into ~/.Iris/skills/
# ============================================================================

Iris_SKILLS_DIR="${Iris_HOME:-$HOME/.Iris}/skills"
mkdir -p "$Iris_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.Iris/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$Iris_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "Other commands:"
echo "  Iris status        # Check configuration"
echo "  Iris gateway install # Install gateway service (messaging + cron)"
echo "  Iris cron list     # View scheduled jobs"
echo "  Iris doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m Iris_cli.main setup
fi
