...
def _handle_send(args):
    ...
    if not pconfig or not pconfig.enabled:
        return json.dumps({"error": f"Platform '{platform_name}' is not configured. Set up credentials in ~/.Iris/gateway.json or environment variables."})
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: Iris config set {platform_name.upper()}_HOME_CHANNEL <channel_id>"
            })
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success"):
            try:
                ...
                source_label = os.getenv("Iris_SESSION_PLATFORM", "cli")
