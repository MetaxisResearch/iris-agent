...
# All skills live in ~/.Iris/skills/ (seeded from bundled skills/ on install).
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.Iris/skills/mlops/axolotl/SKILL.md -> "mlops"
    """
    try:
...
def _get_disabled_skill_names() -> Set[str]:
    ...
    Resolves platform from ``Iris_PLATFORM`` env var, falls back to
    ...
    try:
        ...
        resolved_platform = os.getenv("Iris_PLATFORM")
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("Iris_PLATFORM")
        if resolved_platform:
            platform_disabled = skills_cfg.get("platform_disabled", {}).get(resolved_platform)
            if platform_disabled is not None:
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.Iris/skills/.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``Iris skills`` config UI). Default False
    ...
    try:
        if not SKILLS_DIR.exists():
            ...
            return json.dumps({
                ...
                "skills": [],
                "categories": [],
                "message": "No skills found. Skills directory created at ~/.Iris/skills/"
            }, ensure_ascii=False)
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        # Check metadata.Iris.* first (agentskills.io convention), fall back to top-level
        Iris_meta = {}
        metadata = frontmatter.get('metadata')
        if isinstance(metadata, dict):
            Iris_meta = metadata.get('Iris', {}) or {}
        
        tags = _parse_tags(Iris_meta.get('tags') or frontmatter.get('tags', ''))
        related_skills = _parse_tags(Iris_meta.get('related_skills') or frontmatter.get('related_skills', ''))
