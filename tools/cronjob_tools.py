"""
Cron job management tools for Iris Agent.

These tools allow the agent to schedule, list, and remove automated tasks.
Only available when running via CLI (Iris-cli toolset).
...
def schedule_cronjob(
    ...
    Args:
        ...
        deliver: Where to send the output. Options:
                 ...
                 - "local": Save to local files only (~/.Iris/cron/output/)
    ...
    origin = None
    origin_platform = os.getenv("Iris_SESSION_PLATFORM")
    origin_chat_id = os.getenv("Iris_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        origin = {
            "platform": origin_platform,
            "chat_id": origin_chat_id,
            "chat_name": os.getenv("Iris_SESSION_CHAT_NAME"),
        }
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("Iris_INTERACTIVE")
        or os.getenv("Iris_GATEWAY_SESSION")
        or os.getenv("Iris_EXEC_ASK")
    )
