...
discovers their tools, and registers them into the iris-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.Iris/config.yaml under the ``mcp_servers`` key.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Iris config file.
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Iris registry schema format.
...
def discover_mcp_tools() -> List[str]:
    ...
    if all_tools:
        # Dynamically inject into all Iris-* platform toolsets
        from toolsets import TOOLSETS
        for ts_name, ts in TOOLSETS.items():
            if ts_name.startswith("Iris-"):
