...
Configuration is loaded from ~/.Iris/config.yaml under the 'tts:' key.
...
DEFAULT_OUTPUT_DIR = os.path.expanduser("~/.Iris/audio_cache")
MAX_TEXT_LENGTH = 4000


# ===========================================================================
# Config loader -- reads tts: section from ~/.Iris/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.Iris/config.yaml.
    ...
    try:
        ...
        logger.debug("Iris_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.Iris/config.yaml (tts: section).
    ...
    platform = os.getenv("Iris_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.Iris/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
