...
# Path to tinker-atropos submodule (relative to iris-agent root)
Iris_ROOT = Path(__file__).parent.parent
TINKER_ATROPOS_ROOT = Iris_ROOT / "tinker-atropos"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.Iris/.env",
        }, indent=2)
...
async def rl_check_status(run_id: str) -> str:
    ...
    try:
        ...
        api = wandb.Api()
        runs = api.runs(
            f"{os.getenv('WANDB_ENTITY', 'Metaxis Research')}/{run_state.wandb_project}",
