...
def _check_disk_usage_warning():
    ...
    try:
        # Get total size of Iris directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "Iris-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("Iris_GATEWAY_SESSION")
    ...
    for failure in sudo_failures:
        if failure in output:
            return output + "\n\n💡 Tip: To enable sudo over messaging, add SUDO_PASSWORD to ~/.Iris/.env on the agent machine."
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (Iris_INTERACTIVE=1).
    ...
    try:
        os.environ["Iris_SPINNER_PAUSE"] = "1"
        ...
        if "Iris_SPINNER_PAUSE" in os.environ:
            del os.environ["Iris_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str) -> tuple[str, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (Iris_INTERACTIVE=1):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.Iris/.env or environment variables."
        )
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            session_key = os.getenv("Iris_SESSION_KEY", "")
            ...
            try:
                ...
                if check_interval and background:
                    ...
                    process_registry.pending_watchers.append({
                        ...
                        "session_key": session_key,
                        "platform": os.getenv("Iris_SESSION_PLATFORM", ""),
                        "chat_id": os.getenv("Iris_SESSION_CHAT_ID", ""),
                    })
...
if __name__ == "__main__":
    ...
    print(f"  TERMINAL_SANDBOX_DIR: {os.getenv('TERMINAL_SANDBOX_DIR', '~/.Iris/sandboxes')}")
