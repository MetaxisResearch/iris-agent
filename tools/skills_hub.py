#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Iris Skills Hub.
...
Used by Iris_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            Iris_meta = metadata.get("Iris", {})
            if isinstance(Iris_meta, dict):
                tags = Iris_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Metaxis Research) but not activated
