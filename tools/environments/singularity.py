...
_SNAPSHOT_STORE = Path.home() / ".Iris" / "singularity_snapshots.json"
...
def _get_scratch_dir() -> Path:
    ...
    Resolution order:
      ...
      4. ~/.Iris/sandboxes/singularity (fallback)
    """
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "Iris") / "iris-agent"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.instance_id = f"Iris_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "Iris-overlays"
