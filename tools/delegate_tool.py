...
def _run_single_child(
    ...
    model on OpenRouter while the parent runs on Metaxis Portal).
    """
    ...
    try:
        # Extract parent's API key so subagents inherit auth (e.g. Metaxis Portal).
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, Metaxis, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'Iris login'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (Iris_cli/config.py load_config()) so that
