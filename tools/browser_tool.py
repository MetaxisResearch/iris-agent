...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-Iris_…`` the resulting socket path exceeds the
...
def _create_browserbase_session(task_id: str) -> Dict[str, str]:
    ...
    session_name = f"Iris_{task_id}_{uuid.uuid4().hex[:8]}"
...
def _maybe_start_recording(task_id: str):
    ...
    if task_id in _recording_sessions:
        return
    try:
        Iris_home = Path(os.environ.get("Iris_HOME", Path.home() / ".Iris"))
        ...
        recordings_dir = Iris_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    Iris_home = Path(os.environ.get("Iris_HOME", Path.home() / ".Iris"))
    screenshots_dir = Iris_home / "browser_screenshots"
