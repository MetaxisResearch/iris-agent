...
DANGEROUS_PATTERNS = [
    ...
    (r'\btee\b.*(/etc/|/dev/sd|\.ssh/|\.Iris/\.env)', "overwrite system file via tee"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    session_key = os.getenv("Iris_SESSION_KEY", "default")
    if is_approved(session_key, pattern_key):
        return {"approved": True, "message": None}

    is_cli = os.getenv("Iris_INTERACTIVE")
    is_gateway = os.getenv("Iris_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("Iris_EXEC_ASK"):
