...
# Resolve Iris home directory (respects Iris_HOME override)
_Iris_home = Path(os.getenv("Iris_HOME", Path.home() / ".Iris"))

# Load environment variables from ~/.Iris/.env first
from dotenv import load_dotenv
_env_path = _Iris_home / '.env'
if _env_path.exists():
    try:
...
_config_path = _Iris_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["Iris_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
        # Timezone: bridge config.yaml → Iris_TIMEZONE env var.
        # Iris_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "Iris_TIMEZONE" not in os.environ:
            os.environ["Iris_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["Iris_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Gateway runs in quiet mode - suppress debug output and use cwd directly (no temp dirs)
os.environ["Iris_QUIET"] = "1"
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("Iris_INFERENCE_PROVIDER"),
        )
...
def _resolve_gateway_model() -> str:
    ...
    """
    model = os.getenv("Iris_MODEL") or os.getenv("LLM_MODEL") or "anthropic/claude-opus-4.6"
    try:
        import yaml as _y
        _cfg_path = _Iris_home / "config.yaml"
        if _cfg_path.exists():
...
class GatewayRunner:
    ...
    def _flush_memories_for_session(self, old_session_id: str):
        ...
        SynchroMetaxis worker — meant to be called via run_in_executor from
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks Iris_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.Iris/config.yaml.
        Relative paths are resolved from ~/.Iris/.
        """
        import json as _json
        file_path = os.getenv("Iris_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _Iris_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks Iris_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.Iris/config.yaml.
        """
        prompt = os.getenv("Iris_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        Checks Iris_REASONING_EFFORT env var first, then agent.reasoning_effort
        in config.yaml. Valid: "xhigh", "high", "medium", "low", "minimal", "none".
        Returns None to use default (medium).
        """
        effort = os.getenv("Iris_REASONING_EFFORT", "")
        if not effort:
            try:
                import yaml as _y
                cfg_path = _Iris_home / "config.yaml"
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("Iris_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _Iris_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.Iris/.env to allow open access, "
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'iris-agent[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`Iris pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if history and len(history) >= 4:
            ...
            try:
                _hyg_cfg_path = _Iris_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Iris delivers cron job results "
    ...
    async def _handle_model_command(self, event: MessageEvent) -> str:
        ...
        args = event.get_command_args().strip()
        config_path = _Iris_home / 'config.yaml'

        # Resolve current model and provider from config
        current = os.getenv("Iris_MODEL") or "anthropic/claude-opus-4.6"
        ...
        os.environ["Iris_MODEL"] = new_model
        if provider_changed:
            os.environ["Iris_INFERENCE_PROVIDER"] = target_provider
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.Iris/config.yaml`"
    ...
    async def _run_background_task(
        ...
        try:
            ...
            default_toolset_map = {
                Platform.LOCAL: "Iris-cli",
                Platform.TELEGRAM: "Iris-telegram",
                Platform.DISCORD: "Iris-discord",
                Platform.WHATSAPP: "Iris-whatsapp",
                Platform.SLACK: "Iris-slack",
                Platform.SIGNAL: "Iris-signal",
                Platform.HOMEASSISTANT: "Iris-homeassistant",
                Platform.EMAIL: "Iris-email",
            }
            ...
            if config_toolsets and isinstance(config_toolsets, list):
                ...
                default_toolset = default_toolset_map.get(source.platform, "Iris-telegram")
                enabled_toolsets = [default_toolset]

            platform_key = "cli" if source.platform == Platform.LOCAL else source.platform.value

            pr = self._provider_routing
            max_iterations = int(os.getenv("Iris_MAX_ITERATIONS", "90"))
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Iris Agent to the latest version.

        Spawns ``Iris update`` in a separate systemd scope so it survives the
        gateway restart that ``Iris update`` triggers at the end.  A marker
        ...
        if not Iris_bin:
            return "✗ `Iris` command not found on PATH."

        # Write marker so the restarted gateway can notify this chat
        pending_path = _Iris_home / ".update_pending.json"
        output_path = _Iris_home / ".update_output.txt"
        pending = {
        ...
        # Spawn `Iris update` in a separate cgroup so it survives gateway
        # restart.  systemd-run --user --scope creates a transient scope unit.
        update_cmd = f"{Iris_bin} update > {output_path} 2>&1"
        try:
        ...
        return "⚕ Starting Iris update… I'll notify you when it's done."
        ...
        try:
            ...
            if adapter and chat_id:
                ...
                if output:
                    ...
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    msg = f"✅ Iris update finished — gateway restarted.\n\n```\n{output}\n```"
                else:
                    msg = "✅ Iris update finished — gateway restarted successfully."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["Iris_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["Iris_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["Iris_SESSION_CHAT_NAME"] = context.source.chat_name
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["Iris_SESSION_PLATFORM", "Iris_SESSION_CHAT_ID", "Iris_SESSION_CHAT_NAME"]:
    ...
    async def _run_agent(
        ...
        _progress_cfg = {}
        try:
            _tp_cfg_path = _Iris_home / "config.yaml"
        ...
        progress_mode = (
            ...
            or os.getenv("Iris_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["Iris_SESSION_KEY"] = session_key or ""
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, interval: int = 60):
    ...
    needing a separate `Iris cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False) -> bool:
    ...
    # Prevent two gateways from running under the same Iris_HOME.
    # The PID file is scoped to Iris_HOME, so future multi-profile
    # setups (each profile using a distinct Iris_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            Iris_home = os.getenv("Iris_HOME", "~/.Iris")
            logger.error(
                "Another gateway instance is already running (PID %d, Iris_HOME=%s). "
                "Use 'Iris gateway restart' to replace it, or 'Iris gateway stop' first.",
                existing_pid, Iris_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'Iris gateway restart' to replace it,\n"
                f"   or 'Iris gateway stop' to kill it first.\n"
                f"   Or use 'Iris gateway run --replace' to auto-replace.\n"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Iris Gateway - Multi-platform messaging")
