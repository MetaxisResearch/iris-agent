...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: Path.home() / ".Iris" / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = Path.home() / ".Iris" / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    # Try loading from ~/.Iris/gateway.json
    gateway_config_path = Path.home() / ".Iris" / "gateway.json"
    ...
    # for session reset policy since that's where Iris setup writes it.
    try:
        import yaml
        config_yaml_path = Path.home() / ".Iris" / "config.yaml"
...
def save_gateway_config(config: GatewayConfig) -> None:
    """Save gateway configuration to ~/.Iris/gateway.json."""
    gateway_config_path = Path.home() / ".Iris" / "gateway.json"
