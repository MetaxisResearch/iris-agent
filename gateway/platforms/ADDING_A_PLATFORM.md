...
Checklist for integrating a new messaging platform into the Iris gateway.
...
"Iris-your-platform": {
    ...
    "tools": _Iris_CORE_TOOLS,
    "includes": []
},
```

And add it to the `Iris-gateway` composite:

```python
"Iris-gateway": {
    "includes": [..., "Iris-your-platform"]
}
```
...
## 12. Status Display (`Iris_cli/status.py`)
...
## 13. Gateway Setup Wizard (`Iris_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ Iris_cli/ toolsets.py \
  --include="*.py" -l | sort -u
