...
sidebar_position: 2
title: "Configuration"
description: "Configure Iris Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.Iris/` directory for easy access.
...
├── auth.json       # OAuth provider credentials (Metaxis Portal, etc.)
...
Iris config check        # Check for missing options (after updates)
Iris config migrate      # Interactively add missing options

# Examples:
Iris config set model anthropic/claude-opus-4
Iris config set terminal.backend docker
Iris config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `Iris config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `Iris chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.Iris/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.Iris/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
You need at least one way to connect to an LLM. Use `Iris model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Metaxis Portal** | `Iris model` (OAuth, subscription-based) |
| **OpenAI Codex** | `Iris model` (ChatGPT OAuth, uses Codex models) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.Iris/.env` |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.Iris/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.Iris/.env` (provider: `kimi-coding`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.Iris/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.Iris/.env` (provider: `minimax-cn`) |
| **Custom Endpoint** | `OPENAI_BASE_URL` + `OPENAI_API_KEY` in `~/.Iris/.env` |
...
```bash
# z.ai / ZhipuAI GLM
Iris chat --provider zai --model glm-4-plus
# Requires: GLM_API_KEY in ~/.Iris/.env

# Kimi / Moonshot AI
Iris chat --provider kimi-coding --model moonshot-v1-auto
# Requires: KIMI_API_KEY in ~/.Iris/.env

# MiniMax (global endpoint)
Iris chat --provider minimax --model MiniMax-Text-01
# Requires: MINIMAX_API_KEY in ~/.Iris/.env

# MiniMax (China endpoint)
Iris chat --provider minimax-cn --model MiniMax-Text-01
# Requires: MINIMAX_CN_API_KEY in ~/.Iris/.env
```
...
Iris Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Iris at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
vLLM supports tool calling, structured output, and multi-modal models. Use `--enable-auto-tool-choice` and `--tool-call-parser Iris` for Iris-format tool calling with Metaxis Research models.
...
|----------|-------------|
| **Just want it to work** | OpenRouter (default) or Metaxis Portal |
...
:::tip
You can switch between providers at any time with `Iris model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
By default, Iris uses the [Firecrawl cloud API](https://firecrawl.dev/) for web search and scraping. If you prefer to run Firecrawl locally, you can point Iris at a self-hosted instance instead.
...
2. Point Iris at your instance (no API key needed):
   ```bash
   Iris config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.Iris/config.yaml`:
...
```yaml
worktree: true    # Always create a worktree (same as Iris -w)
...
compression:
  ...
  # summary_provider: "auto"   # "auto", "openrouter", "Metaxis", "main"
```
...
Iris uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via OpenRouter or Metaxis Portal — you don't need to configure anything.

To use a different model, add an `auxiliary` section to `~/.Iris/config.yaml`:

```yaml
auxiliary:
  # Image analysis (vision_analyze tool + browser screenshots)
  vision:
    provider: "auto"           # "auto", "openrouter", "Metaxis", "main"
...
Or via environment variable (in `~/.Iris/.env`):
...
|----------|-------------|-------------|
| `"auto"` | Best available (default). Vision tries OpenRouter → Metaxis → Codex. | — |
| `"openrouter"` | Force OpenRouter — routes to any model (Gemini, GPT-4o, Claude, etc.) | `OPENROUTER_API_KEY` |
| `"Metaxis"` | Force Metaxis Portal | `Iris login` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `Iris model` → Codex |
...
:::tip
Run `Iris config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
quick_commands:
  status:
    type: exec
    command: systemctl status iris-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.Iris/iris-agent && git pull && pip install -e .
  gpu:
    type: exec
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.Iris/browser_recordings/
```
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: Iris --checkpoints)
...
The delegation provider uses the same credential resolution as CLI/gateway startup. All configured providers are supported: `openrouter`, `Metaxis`, `zai`, `kimi-coding`, `minimax`, `minimax-cn`. When a provider is set, the system automatically resolves the correct base URL, API key, and API mode — no manual credential wiring needed.
...
- **SOUL.md** checks cwd first, then `~/.Iris/SOUL.md` as a global fallback.
...
| Context | Default |
|---------|---------|
| **CLI (`Iris`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.Iris/.env or ~/.Iris/config.yaml:
