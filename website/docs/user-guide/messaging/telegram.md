...
sidebar_position: 1
title: "Telegram"
description: "Set up Iris Agent as a Telegram bot"
---
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "Iris Agent") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_Iris_bot`)
...
Iris Agent uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
You can also set it manually in `~/.Iris/.env`:
...
Voice messages you send on Telegram are automatically transcribed using OpenAI's Whisper API and injected as text into the conversation. This requires `VOICE_TOOLS_OPENAI_KEY` in `~/.Iris/.env`.
...
Iris Agent works in Telegram group chats with a few considerations:

- **Privacy mode** determines what messages the bot can see (see [Step 3](#step-3-privacy-mode-critical-for-groups))
- When privacy mode is on, **@mention the bot** (e.g., `@my_Iris_bot what's the weather?`) or **reply to its messages** to interact
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `Iris gateway` logs for errors. |
...
| Voice messages not transcribed | Check that `VOICE_TOOLS_OPENAI_KEY` is set and valid in `~/.Iris/.env`. |
