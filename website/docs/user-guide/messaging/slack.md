...
sidebar_position: 4
title: "Slack"
description: "Set up Iris Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Iris Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Iris instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Iris uses the modern
...
4. Enter an app name (e.g., "Iris Agent") and select your workspace
5. Click **Create App**
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `Iris-socket` (the name doesn't matter)
...
Iris uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
## Step 7: Configure Iris

Add the following to your `~/.Iris/.env` file:
...
```bash
Iris gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
Iris gateway              # Foreground
Iris gateway install      # Install as a system service
```
...
Understanding how Iris behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Iris Agent what time is it?`) |
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Iris will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Iris Agent`).

---

## Voice Messages

Iris supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Iris Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Iris Agent` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Iris Agent`)
...
- Tokens should be stored in `~/.Iris/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Iris config directory
