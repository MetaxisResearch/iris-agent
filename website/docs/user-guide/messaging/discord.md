...
3. Enter a name for your application (e.g., "Iris Agent") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential Iris Agent uses to log in as your bot. Still on the **Bot** page:
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the Iris gateway).

## Step 7: Find Your Discord User ID

Iris Agent uses your Discord User ID to control who can interact with the bot. To find it:
...
:::tip
You can run `Iris gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::
...
Iris Agent supports Discord voice messages:
...
**Cause**: The Iris gateway isn't running, or the token is incorrect.

**Fix**: Check that `Iris gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.Iris/.env` and restart the gateway.
...
For more information on securing your Iris Agent deployment, see the [Security Guide](../security.md).
