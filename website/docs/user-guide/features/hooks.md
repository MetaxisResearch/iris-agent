...
Each hook is a directory under `~/.Iris/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".Iris" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.Iris/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.Iris/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.Iris/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.Iris/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".Iris" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.Iris/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.Iris/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/Iris-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.Iris/hooks/`
