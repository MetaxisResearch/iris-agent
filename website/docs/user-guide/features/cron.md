...
```bash
Iris gateway install     # Install as system service (recommended)
Iris gateway             # Or run in foreground

Iris cron list           # View scheduled jobs
Iris cron status         # Check if gateway is running
```
...
1. It loads all jobs from `~/.Iris/cron/jobs.json`
...
A **file-based lock** (`~/.Iris/cron/.tick.lock`) prevents duplicate execution if multiple processes overlap (e.g., gateway + manual tick).
...
| `"local"` | Save to local files only (`~/.Iris/cron/output/`) | Default on CLI |
...
**How `"origin"` works:** When a job is created from a messaging platform, Iris records the source platform and chat ID. When the job runs and deliver is `"origin"`, the output is sent back to that exact platform and chat. If origin info isn't available (e.g., job created from CLI), delivery falls back to local.

**How platform names work:** When you specify a bare platform name like `"telegram"`, Iris first checks if the job's origin matches that platform and uses the origin chat ID. Otherwise, it falls back to the platform's home channel configured via environment variable (e.g., `TELEGRAM_HOME_CHANNEL`).
...
Iris cron status         # Check if the scheduler is running
...
Jobs are stored as JSON in `~/.Iris/cron/jobs.json`. Output from job runs is saved to `~/.Iris/cron/output/{job_id}/{timestamp}.md`.
