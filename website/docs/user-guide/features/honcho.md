...
[Honcho](https://honcho.dev) is an AI-native memory system that gives Iris Agent persistent, cross-session understanding of users. While Iris has built-in memory (`MEMORY.md` and `USER.md` files), Honcho adds a deeper layer of **user modeling** — learning user preferences, goals, communication style, and context across conversations.

## How It Complements Built-in Memory

Iris has two memory systems that work together:

| Feature | Built-in Memory | Honcho Memory |
|---------|----------------|---------------|
| Storage | Local files (`~/.Iris/memories/`) | Cloud-hosted Honcho API |
...
| `workspace` | `"Iris"` | Workspace identifier |
| `peerName` | *(derived)* | Your identity name for user modeling |
| `aiPeer` | `"Iris"` | AI assistant identity name |
...
### Iris Config (`~/.Iris/config.yaml`)

The `honcho` section in Iris config is intentionally minimal — most configuration comes from the global `~/.honcho/config.json`:
...
When Honcho is active, Iris gains access to the `query_user_context` tool. This lets the agent proactively ask Honcho about the user during conversations:
...
- **Context prefetch** — before responding, Iris can prefetch user context (representation + peer card) in a single API call
