...
description: "Overview of Iris Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Iris chat --toolsets "web,terminal"
...
# Configure
Iris config set terminal.backend singularity
Iris config set terminal.singularity_image ~/python.sif
```
...
modal setup
Iris config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.Iris/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.Iris/.env`.
:::
