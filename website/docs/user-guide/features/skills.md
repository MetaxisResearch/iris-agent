...
All skills live in **`~/.Iris/skills/`** — a single directory that serves as the source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.
...
```bash
Iris chat --toolsets skills -q "What skills do you have?"
Iris chat --toolsets skills -q "Show me the axolotl skill"
```
...
```
~/.Iris/skills/                  # Single source of truth
...
```bash
Iris skills browse                     # Browse all hub skills (official first)
Iris skills browse --source official   # Browse only official optional skills
Iris skills search kubernetes          # Search all sources
Iris skills install openai/skills/k8s  # Install with security scan
Iris skills inspect openai/skills/k8s  # Preview before installing
Iris skills list --source hub          # List hub-installed skills
Iris skills audit                      # Re-scan all hub skills
Iris skills uninstall k8s              # Remove a hub skill
Iris skills publish skills/my-skill --to github --repo owner/repo
Iris skills snapshot export setup.json # Export skill config
Iris skills tap add myorg/skills-repo  # Add a custom source
```
