...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Iris Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Iris Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
Iris chat --model "anthropic/claude-sonnet-4"

# With a specific provider
Iris chat --provider Metaxis        # Use Metaxis Portal
Iris chat --provider openrouter  # Force OpenRouter

# With specific toolsets
Iris chat --toolsets "web,terminal,skills"

# Resume previous sessions
Iris --continue             # Resume the most recent CLI session (-c)
Iris --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
Iris chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
Iris -w                         # Interactive mode in worktree
Iris -w -q "Fix issue #123"     # Single query in worktree
```
...
When resuming a previous session (`Iris -c` or `Iris --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
Every installed skill in `~/.Iris/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.Iris/config.yaml`:
...
agent:
  personalities:
    ...
    pirate: "Arrr! Ye be talkin' to Captain Iris..."
    # Add your own!
```
...
Resume this session with:
  Iris --resume 20260225_143052_a1b2c3
...
```bash
Iris --continue                          # Resume the most recent CLI session
Iris -c                                  # Short form
Iris -c "my project"                     # Resume a named session (latest in lineage)
Iris --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
Iris --resume "refactoring auth"         # Resume by title
Iris -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `Iris sessions rename <id> <title>` from the command line. Use `Iris sessions list` to browse past sessions.

### Session Logging

Sessions are automatically logged to `~/.Iris/sessions/`:
