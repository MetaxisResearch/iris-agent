...
Iris Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.Iris/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.Iris/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`Iris` or `Iris chat`) |
...
Iris -c "my project"   # → resumes "my project #3"
```
...
Iris --resume 20250305_091523_a1b2c3d4
Iris -r 20250305_091523_a1b2c3d4

# Resume by title
Iris --resume "refactoring auth"

# Or with the chat subcommand
Iris chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `Iris sessions list`.

### Conversation Recap on Resume

When you resume a session, Iris displays a compact recap of the previous conversation in a styled panel before the input prompt:

```text
╭─────────────────────────── Previous Conversation ────────────────────────────╮
│   ● You: What is Python?                                                     │
│   ◆ Iris: Python is a high-level programming language.                     │
│   ● You: How do I install it?                                                │
│   ◆ Iris: [3 tool calls: web_search, web_extract, terminal]                │
│   ◆ Iris: You can download Python from python.org...                       │
╰──────────────────────────────────────────────────────────────────────────────╯
```
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.Iris/config.yaml`:
...
```bash
Iris sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Iris creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`Iris -c "my project"`), it automatically picks the most recent session in the lineage.
...
Iris provides a full set of session management commands via `Iris sessions`:
...
# Filter by platform
Iris sessions list --source telegram

# Show more sessions
Iris sessions list --limit 50
```
...
Iris sessions export backup.jsonl

# Export sessions from a specific platform
Iris sessions export telegram-history.jsonl --source telegram

# Export a single session
Iris sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
Iris sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
Iris sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
Iris sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
Iris sessions prune --older-than 30

# Only prune sessions from a specific platform
Iris sessions prune --source telegram --older-than 60

# Skip confirmation
Iris sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`Iris insights`](/docs/reference/cli-commands#insights).
...
|------|------|-------------|
| SQLite database | `~/.Iris/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.Iris/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.Iris/sessions/sessions.json` | Maps session keys to active session IDs |
...
Iris sessions delete <session_id>
