...
2. **Iris spins up** a fresh agent session with your prompt
...
- **Iris Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.Iris/cron/output/` and you can read them anytime.
:::
...
Iris will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Iris Agent
```
...
Iris will create the cron job for you using the `schedule_cronjob` tool.
...
For faster briefings, tell Iris to delegate each topic to a sub-agent:
...
Iris will use `list_cronjobs` to find it and `remove_cronjob` to delete it.
