...
This tutorial walks you through setting up a Telegram bot powered by Iris Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Iris Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/learning-path) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.Iris/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Iris itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Iris Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_Iris_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Iris Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u Iris-gateway -f
...
launchctl start ai.Iris.gateway
launchctl stop ai.Iris.gateway
tail -f ~/.Iris/logs/gateway.log
```
...
```bash
Iris gateway stop && Iris gateway start
```
...
3. **You approve it** on the server:
   ```bash
   Iris pairing approve telegram XKGH5N7P
   ```
...
Iris pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.Iris/.env`:
...
Control how much detail the bot shows when using tools. In `~/.Iris/config.yaml`:
...
Customize how the bot communicates by creating `~/.Iris/SOUL.md`:
...
```bash
# From the CLI
Iris cron list          # View all scheduled jobs
Iris cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u Iris-gateway` (Linux) or `~/.Iris/logs/gateway.log` (macOS) |
| Cron job output | `~/.Iris/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.Iris/cron/jobs.json` |
| Pairing data | `~/.Iris/pairing/` |
| Session history | `~/.Iris/sessions/` |
