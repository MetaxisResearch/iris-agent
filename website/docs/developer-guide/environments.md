...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the iris-agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Iris Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/Metaxis Research/atropos) RL training framework. This enables three workflows:
...
```
                 ...
                 └───────────┬───────────┘
                             │ inherits
                 ┌───────────┴───────────┐
                 │  IrisAgentBaseEnv    │  environments/Iris_base_env.py
                 │  - Terminal backend    │
                 │  - Tool resolution     │
                 │  - Agent loop engine   │
                 │  - ToolContext         │
       ...
       ┌─────────────────────┼─────────────────────┐
       │                     │                      │
  TerminalTestEnv     IrisSweEnv     TerminalBench2EvalEnv
...
The iris-agent layer (`environments/Iris_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls iris-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `IrisAgentLoop` and scores the result
...
Your environment inherits from `IrisAgentBaseEnv` and implements five methods:
...
`IrisAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as iris-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any iris-agent tool |
...
parser = get_parser("Iris")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `Iris`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
Dataset: [Metaxis Research/terminal-bench-2](https://huggingface.co/datasets/Metaxis Research/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [Metaxis Research/openthoughts-tblite](https://huggingface.co/datasets/Metaxis Research/openthoughts-tblite).
...
pip install "iris-agent[yc-bench]"
...
```bash
python environments/Iris_swe_env/Iris_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"Iris"`, `"qwen"`, `"mistral"`)
...
```python
from environments.Iris_base_env import IrisAgentBaseEnv, IrisAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(IrisAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(IrisAgentBaseEnv):
    name = "my-env"
...
### IrisAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which Iris toolsets to enable |
...
| `tool_call_parser` | `str` | `"Iris"` | Parser for Phase 2 raw output |
...
env:
  ...
  dataset_name: "Metaxis Research/terminal-bench-2"
  tokenizer_name: "Metaxis Research/Iris-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/Metaxis Research/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "iris-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "iris-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── Iris_base_env.py          # Abstract base class (IrisAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (IrisAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── Iris_parser.py        # Iris/ChatML <tool_call> format
...
├── Iris_swe_env/             # SWE-bench training environment
│
