...
sidebar_position: 1
title: "Architecture"
description: "Iris Agent internals — project structure, agent loop, key classes, and design patterns"
---

# Architecture

This guide covers the internal architecture of Iris Agent for developers contributing to the project.
...
├── cli.py                    # IrisCLI class — interactive TUI, prompt_toolkit
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets
├── Iris_state.py           # SQLite session database with FTS5 full-text search
...
│
├── skills/                   # Bundled skills (copied to ~/.Iris/skills/)
...
All conversations are stored in SQLite (`Iris_state.py`) with full-text search. JSON logs go to `~/.Iris/sessions/`.
...
The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Metaxis Portal OAuth, OpenRouter API key, or custom endpoint).
