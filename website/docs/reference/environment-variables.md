...
description: "Complete reference of all environment variables used by Iris Agent"
---

# Environment Variables Reference

All variables go in `~/.Iris/.env`. You can also set them with `Iris config set VAR value`.
...
| `Iris_MODEL` | Preferred model name (checked before `LLM_MODEL`, used by gateway) |
| `LLM_MODEL` | Default model name (fallback when not set in config.yaml) |
| `VOICE_TOOLS_OPENAI_KEY` | OpenAI key for TTS and voice transcription (separate from custom endpoint) |
| `Iris_HOME` | Override Iris config directory (default: `~/.Iris`) |

## Provider Auth (OAuth)

| Variable | Description |
|----------|-------------|
| `Iris_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `Metaxis`, `zai`, `kimi-coding`, `minimax`, `minimax-cn` (default: `auto`) |
| `Iris_PORTAL_BASE_URL` | Override Metaxis Portal URL (for development/testing) |
| `Metaxis_INFERENCE_BASE_URL` | Override Metaxis inference API URL |
| `Iris_Metaxis_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `Iris_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.Iris/sandboxes/`) |
...
| Variable | Description |
|----------|-------------|
| `Iris_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 60) |
| `Iris_TOOL_PROGRESS` | Send progress messages when using tools (`true`/`false`) |
| `Iris_TOOL_PROGRESS_MODE` | `all` (every call, default) or `new` (only when tool changes) |
| `Iris_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `Iris_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `Iris_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
| `Iris_QUIET` | Suppress non-essential output (`true`/`false`) |
| `Iris_EXEC_ASK` | Enable execution approval prompts in gateway mode (`true`/`false`) |
...
These go in `~/.Iris/config.yaml` under the `provider_routing` section:
...
:::tip
Use `Iris config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
