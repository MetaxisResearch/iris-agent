"""Iris CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in ~/.Iris/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Iris Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Iris "       # Response box header label
...
.. code-block:: python
    ...
    from Iris_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Iris Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.Iris/skins/
...
Drop a YAML file in ``~/.Iris/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    tool_prefix: str = "┊"
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces Iris_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces Iris_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Iris — gold and kawaii",
        "colors": {
...
def _skins_dir() -> Path:
    ...
    home = Path(os.getenv("Iris_HOME", Path.home() / ".Iris"))
    return home / "skins"
