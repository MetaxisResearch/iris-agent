"""
Interactive setup wizard for Iris Agent.
...
Config files are stored in ~/.Iris/ for easy access.
"""
...
from Iris_cli.colors import Colors, color
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with 'Iris setup')")


def _print_setup_summary(config: dict, Iris_home):
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'Iris setup tools' to configure them,"
        )
        print_warning("or edit ~/.Iris/.env directly to add the missing API keys.")
        print()
    ...
    print(color("📁 All your files are in ~/.Iris/:", Colors.CYAN, Colors.BOLD))
    print()
    print(f"   {color('Settings:', Colors.YELLOW)}  {get_config_path()}")
    print(f"   {color('API Keys:', Colors.YELLOW)}  {get_env_path()}")
    print(
        f"   {color('Data:', Colors.YELLOW)}      {Iris_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('Iris setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('Iris setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('Iris setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('Iris setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('Iris setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('Iris config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('Iris config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('Iris config set KEY VALUE', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('Iris', Colors.GREEN)}              Start chatting")
    print(f"   {color('Iris gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('Iris doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `Iris tools` and `Iris setup tools`).
...
def setup_model_provider(config: dict):
    ...
    from Iris_cli.auth import (
        ...
        format_auth_error,
        AuthError,
        fetch_Metaxis_models,
        resolve_Metaxis_runtime_credentials,
    ...
    provider_choices = [
        "Login with Metaxis Portal (Metaxis Research subscription — OAuth)",
    ...
    if not has_any_provider:
        print_warning("An inference provider is required for Iris to work.")
        print()
    ...
    selected_provider = (
        None  # "Metaxis", "openai-codex", "openrouter", "custom", or None (keep)
    )
    Metaxis_models = []  # populated if Metaxis login succeeds

    if provider_idx == 0:  # Metaxis Portal (OAuth)
        selected_provider = "Metaxis"
        print()
        print_header("Metaxis Portal Login")
        print_info("This will open your browser to authenticate with Metaxis Portal.")
        print_info("You'll need a Metaxis Research account with an active subscription.")
        print()

        try:
            from Iris_cli.auth import _login_Metaxis, ProviderConfig
            import argparse
            ...
            pconfig = PROVIDER_REGISTRY["Metaxis"]
            _login_Metaxis(mock_args, pconfig)
            _sync_model_from_disk(config)

            # Fetch models for the selection step
            try:
                creds = resolve_Metaxis_runtime_credentials(
                    min_key_ttl_seconds=5 * 60,
                    timeout_seconds=15.0,
                )
                Metaxis_models = fetch_Metaxis_models(
                    inference_base_url=creds.get("base_url", ""),
                    api_key=creds.get("api_key", ""),
                )
            except Exception as e:
                logger.debug("Could not fetch Metaxis models after login: %s", e)

        except SystemExit:
            print_warning("Metaxis Portal login was cancelled or failed.")
            print_info("You can try again later with: Iris model")
        ...
        try:
            from Iris_cli.auth import deactivate_provider
        ...
        config_path = (
            Path(os.environ.get("Iris_HOME", Path.home() / ".Iris")) / "config.yaml"
        )
        try:
            disk_cfg = {}
        ...
        if base_url:
            ...
            config_path = (
                Path(os.environ.get("Iris_HOME", Path.home() / ".Iris"))
                / "config.yaml"
            )
            try:
                disk_cfg = {}
        ...
        if api_key:
            ...
            from Iris_cli.auth import detect_zai_endpoint
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "Metaxis" and Metaxis_models:
            # Dynamic model list from Metaxis Portal
            model_choices = [f"{m}" for m in Metaxis_models]
            model_choices.append("Custom model")
            model_choices.append(f"Keep current ({current_model})")

            # Post-login validation: warn if current model might not be available
            if current_model and current_model not in Metaxis_models:
                print_warning(
                    f"Your current model ({current_model}) may not be available via Metaxis Portal."
                )
                print_info(
            ...
            if model_idx < len(Metaxis_models):
                _set_default_model(config, Metaxis_models[model_idx])
            ...
            # Metaxis login succeeded but model fetch failed — prompt manually
            # instead of falling through to the OpenRouter static list.
            print_warning("Could not fetch available models from Metaxis Portal.")
            print_info("Enter a Metaxis model name manually (e.g., claude-opus-4-6).")
            ...
            from Iris_cli.models import model_ids, menu_labels
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Iris runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Iris via Telegram/Discord, this is where")
        print_info(
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("Iris_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("Iris_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Iris from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where Iris delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://iris-agent.ai/docs/user-guide/messaging/slack"
        )
        print()
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'Iris whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'Iris whatsapp' to choose your mode (separate bot number")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     Iris config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: Iris gateway install")
            else:
                print_info("  You can install later: Iris gateway install")
                print_info("  Or run in foreground:  Iris gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   Iris gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `Iris setup tools` and `Iris tools` use the same flow:
...
def _offer_openclaw_migration(Iris_home: Path) -> bool:
    ...
    print_info("Iris can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_Iris", _OPENCLAW_SCRIPT
        )
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Iris.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      Iris setup           — full or quick (auto-detected)
      Iris setup model     — just model/provider
      Iris setup terminal  — just terminal backend
      Iris setup gateway   — just messaging platforms
      Iris setup tools     — just tool configuration
      Iris setup agent     — just agent settings
    """
    ensure_Iris_home()
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Iris Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    from Iris_cli.auth import get_active_provider
    ...
    print(
        color(
            "│             ⚕ Iris Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Iris Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Iris configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, Iris_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'Iris setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'Iris setup' again when ready.")
            return
            ...
            func(config)
            save_config(config)
            _print_setup_summary(config, Iris_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        if _offer_openclaw_migration(Iris_home):
    ...
    print_info(f"Data folder:  {Iris_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'Iris config edit'")
...
def _run_quick_setup(config: dict, Iris_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'Iris setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Iris to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'Iris setup gateway'.")
