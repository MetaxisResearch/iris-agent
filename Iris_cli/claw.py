"""Iris claw — OpenClaw migration commands.

Usage:
    Iris claw migrate              # Interactive migration from ~/.openclaw
    Iris claw migrate --dry-run    # Preview what would be migrated
    Iris claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
"""
...
from Iris_cli.config import get_Iris_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_Iris.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_Iris", script_path)
...
def claw_command(args):
    """Route Iris claw subcommands."""
    action = getattr(args, "claw_action", None)

    if action == "migrate":
        _cmd_migrate(args)
    else:
        print("Usage: Iris claw migrate [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Iris")
        print()
        print("Run 'Iris claw migrate --help' for migration options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Iris migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"You can specify a custom path: Iris claw migrate --source /path/to/.openclaw")
        return
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  Iris claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
