"""
Multi-provider authentication system for Iris Agent.

Supports OAuth device code flows (Metaxis Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.Iris/auth.json with cross-process file locking.
...
from Iris_cli.config import get_Iris_home, get_config_path
...
# Metaxis Portal defaults
DEFAULT_Metaxis_PORTAL_URL = "https://portal.Metaxis Research.com"
DEFAULT_Metaxis_INFERENCE_URL = "https://inference-api.Metaxis Research.com/v1"
DEFAULT_Metaxis_CLIENT_ID = "Iris-cli"
DEFAULT_Metaxis_SCOPE = "inference:mint_agent_key"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "Metaxis": ProviderConfig(
        id="Metaxis",
        name="Metaxis Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_Metaxis_PORTAL_URL,
        inference_base_url=DEFAULT_Metaxis_INFERENCE_URL,
        client_id=DEFAULT_Metaxis_CLIENT_ID,
        scope=DEFAULT_Metaxis_SCOPE,
    ),
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `Iris model` to re-authenticate."

    if error.code == "subscription_required":
        return (
            "No active paid subscription found on Metaxis Portal. "
    ...
    if error.code == "insufficient_credits":
        return (
            ...
            "Top up/renew credits in Metaxis Portal, then retry."
        )
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("Iris_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.Iris/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_Iris_home() / "auth.json"
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "Metaxis_portal" in systems:
            providers["Metaxis"] = systems["Metaxis_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "Metaxis" if providers else None}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `Iris logout`.
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.Iris/auth.json (not ~/.codex/)
#
# Iris maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Iris auth store (~/.Iris/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `Iris login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `Iris login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `Iris login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `Iris login` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Iris auth store (~/.Iris/auth.json)."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Iris auth store automatically.
    """
    refresh_token = tokens.get("refresh_token")
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Iris's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Iris auth store")
            print("⚠️  Migrating Codex credentials to Iris's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `Iris login` to create a fully independent session.\n")
    ...
    refresh_timeout_seconds = float(os.getenv("Iris_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Iris processes
    ...
    base_url = (
        os.getenv("Iris_CODEX_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# Metaxis Portal — token refresh, agent key minting, model discovery
# =============================================================================
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="Metaxis", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="Metaxis", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token"}
    raise AuthError(description, provider="Metaxis", code=code, relogin_required=relogin)
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "api_key" not in payload:
            raise AuthError("Mint response missing api_key",
                            provider="Metaxis", code="server_error")
        return payload
    ...
    try:
        ...
        raise AuthError("Agent key mint request failed",
                        provider="Metaxis", code="server_error") from exc
...
def fetch_Metaxis_models(
    *,
    ...
    """Fetch available model IDs from the Metaxis inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="Metaxis", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Iris models — they're not reliable for agentic tool-calling
            if "Iris" in mid.lower():
                continue
...
def resolve_Metaxis_runtime_credentials(
    *,
    ...
    """
    Resolve Metaxis inference credentials for runtime use.
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "Metaxis")

        if not state:
            raise AuthError("Iris is not logged into Metaxis Portal.",
                            provider="Metaxis", relogin_required=True)

        portal_base_url = (
            _optional_base_url(state.get("portal_base_url"))
            or os.getenv("Iris_PORTAL_BASE_URL")
            or os.getenv("Metaxis_PORTAL_BASE_URL")
            or DEFAULT_Metaxis_PORTAL_URL
        ).rstrip("/")
        inference_base_url = (
            _optional_base_url(state.get("inference_base_url"))
            or os.getenv("Metaxis_INFERENCE_BASE_URL")
            or DEFAULT_Metaxis_INFERENCE_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_Metaxis_CLIENT_ID)

        def _persist_state(reason: str) -> None:
            try:
                _save_provider_state(auth_store, "Metaxis", state)
                _save_auth_store(auth_store)
            except Exception as exc:
                _oauth_trace(
                    "Metaxis_state_persist_failed",
        ...
        _oauth_trace(
            "Metaxis_runtime_credentials_start",
            sequence_id=sequence_id,
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Metaxis Portal login.",
                                provider="Metaxis", relogin_required=True)
        ...
        _persist_state("resolve_Metaxis_runtime_credentials_final")

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Metaxis inference API key",
                        provider="Metaxis", code="server_error")
...
def get_Metaxis_auth_status() -> Dict[str, Any]:
    """Status snapshot for `Iris status` output."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target == "Metaxis":
        return get_Metaxis_auth_status()
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that Iris can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- Iris provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `Iris login` to create a separate session",
        })
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from Iris_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'Iris model' or 'Iris setup' instead."""
    print("The 'Iris login' command has been removed.")
    print("Use 'Iris model' to select a provider and model,")
    print("or 'Iris setup' for full interactive setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.Iris/auth.json."""

    # Check for existing Iris-owned credentials
    try:
        existing = resolve_codex_runtime_credentials()
        print("Existing Codex credentials found in Iris auth store.")
        try:
    ...
    if cli_tokens:
        ...
        print("Iris will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("Iris_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Iris will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Iris gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Iris creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Iris auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    print(f"  Auth state: ~/.Iris/auth.json")
    print(f"  Config updated: {config_path} (model.provider=openai-codex)")
...
def _login_Metaxis(args, pconfig: ProviderConfig) -> None:
    """Metaxis Portal device authorization flow."""
    portal_base_url = (
    ...
    print(f"Starting Iris login via {pconfig.name}...")
    ...
    try:
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "Metaxis", auth_state)
            saved_to = _save_auth_store(auth_store)

        config_path = _update_config_for_provider("Metaxis", inference_base_url)
        print()
        print("Login successful!")
        print(f"  Auth state: {saved_to}")
        print(f"  Config updated: {config_path} (model.provider=Metaxis)")

        # Mint an initial agent key and list available models
        try:
            runtime_creds = resolve_Metaxis_runtime_credentials(
            ...
            if not isinstance(runtime_key, str) or not runtime_key:
                raise AuthError("No runtime API key available to fetch models",
                                provider="Metaxis", code="invalid_token")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Iris will use OpenRouter for inference.")
        else:
            print("Run `Iris model` or configure an API key to use Iris.")
    else:
