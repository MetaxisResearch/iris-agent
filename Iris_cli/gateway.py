"""
Gateway subcommand for Iris CLI.

Handles: Iris gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
SERVICE_NAME = "Iris-gateway"
SERVICE_DESCRIPTION = "Iris Agent Gateway - Messaging Platform Integration"

def get_systemd_unit_path() -> Path:
    return Path.home() / ".config" / "systemd" / "user" / f"{SERVICE_NAME}.service"

def get_launchd_plist_path() -> Path:
    return Path.home() / "Library" / "LaunchAgents" / "ai.Iris.gateway.plist"
...
def get_Iris_cli_path() -> str:
    """Get the path to the Iris CLI."""
    ...
    return f"{get_python_path()} -m Iris_cli.main"
...
def generate_systemd_unit() -> str:
    ...
    Iris_cli = shutil.which("Iris") or f"{python_path} -m Iris_cli.main"
    return f"""[Unit]
...
[Service]
Type=simple
ExecStart={python_path} -m Iris_cli.main gateway run --replace
ExecStop={Iris_cli} gateway stop
...
def systemd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print(f"  Iris gateway start              # Start the service")
    print(f"  Iris gateway status             # Check status")
...
def systemd_status(deep: bool = False):
    ...
    if not unit_path.exists():
        ...
        print("  Run: Iris gateway install")
        return
...
<dict>
    <key>Label</key>
    <string>ai.Iris.gateway</string>
    
    <key>ProgramArguments</key>
    <array>
        <string>{python_path}</string>
        <string>-m</string>
        <string>Iris_cli.main</string>
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  Iris gateway status             # Check status")
    print("  tail -f ~/.Iris/logs/gateway.log  # View logs")
...
def launchd_start():
    subprocess.run(["launchctl", "start", "ai.Iris.gateway"], check=True)
    print("✓ Service started")

def launchd_stop():
    subprocess.run(["launchctl", "stop", "ai.Iris.gateway"], check=True)
    print("✓ Service stopped")
...
def launchd_status(deep: bool = False):
    result = subprocess.run(
        ["launchctl", "list", "ai.Iris.gateway"],
        capture_output=True,
        text=True
    )
    ...
    if deep:
        log_file = Path.home() / ".Iris" / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Iris agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Iris will use (e.g., Iris@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = Path.home() / ".Iris" / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'Iris pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: Iris pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'Iris gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from Iris_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"IrisAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: Iris gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: Iris gateway")
    else:
        print()
        print_info("No platforms configured. Run 'Iris gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: Iris gateway run")
            sys.exit(1)
        ...
        if is_linux() and get_systemd_unit_path().exists():
            ...
            if pids:
                ...
                print("  Iris gateway install")
            else:
                print("✗ Gateway is not running")
                print()
                print("To start:")
                print("  Iris gateway          # Run in foreground")
                print("  Iris gateway install  # Install as service")
