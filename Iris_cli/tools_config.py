"""
Unified tool configuration for Iris Agent.

`Iris tools` and `Iris setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.Iris/config.yaml under
...
# They're still in _Iris_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl"}

# Platform display config
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "Iris-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "Iris-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "Iris-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "Iris-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "Iris-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "Iris-signal"},
    "email":    {"label": "📧 Email",      "default_toolset": "Iris-email"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning("    npm install failed - run manually: cd ~/.Iris/iris-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in iris-agent directory)")
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str]) -> Set[str]:
    ...
    from Iris_cli.curses_ui import curses_checklist
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `Iris tools` and `Iris setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Iris Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    print(color("  Tool configuration saved to ~/.Iris/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'Iris' or gateway restart.", Colors.DIM))
    print()
