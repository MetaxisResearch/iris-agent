...
Add, remove, or reorder entries here — both `Iris setup` and
`Iris` provider-selection will pick up the change automatically.
"""
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    _PROVIDER_ORDER = [
        "openrouter", "Metaxis", "openai-codex",
...
def curated_models_for_provider(provider: Optional[str]) -> list[tuple[str, str]]:
    ...
    # Try live API first (Codex, Metaxis, etc. all support /models)
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Iris' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``Iris_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Metaxis),
    ...
    if normalized == "Metaxis":
        # Try live Metaxis Portal /models endpoint
        try:
            from Iris_cli.auth import fetch_Metaxis_models, resolve_Metaxis_runtime_credentials
            creds = resolve_Metaxis_runtime_credentials()
            if creds:
                live = fetch_Metaxis_models(creds.get("api_key", ""), creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
