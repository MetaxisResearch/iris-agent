...
Usage:
    Iris pairing list              # Show all pending + approved users
    Iris pairing approve <platform> <code>  # Approve a pairing code
    Iris pairing revoke <platform> <user_id> # Revoke user access
    Iris pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle Iris pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: Iris pairing {list|approve|revoke|clear-pending}")
        print("Run 'Iris pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  Run 'Iris pairing list' to see pending codes.\n")
