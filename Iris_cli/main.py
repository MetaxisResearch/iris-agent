#!/usr/bin/env python3
"""
Iris CLI - Main entry point.

Usage:
    Iris                     # Interactive chat (default)
    Iris chat                # Interactive chat
    Iris gateway             # Run gateway in foreground
    Iris gateway start       # Start gateway as service
    Iris gateway stop        # Stop gateway service
    Iris gateway status      # Show gateway status
    Iris gateway install     # Install gateway service
    Iris gateway uninstall   # Uninstall gateway service
    Iris setup               # Interactive setup wizard
    Iris logout              # Clear stored authentication
    Iris status              # Show status of all components
    Iris cron                # Manage cron jobs
    Iris cron list           # List cron jobs
    Iris cron status         # Check if cron scheduler is running
    Iris doctor              # Check configuration and dependencies
    Iris version             # Show version
    Iris update              # Update to latest version
    Iris uninstall           # Uninstall Iris Agent
    Iris sessions browse     # Interactive session picker with search
    Iris claw migrate        # Migrate from OpenClaw to Iris
    Iris claw migrate --dry-run  # Preview migration without changes
"""
...
# Load .env from ~/.Iris/.env first, then project root as dev fallback
from dotenv import load_dotenv
from Iris_cli.config import get_env_path, get_Iris_home
...
# Point mini-swe-agent at ~/.Iris/ so it shares our config
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(get_Iris_home()))
os.environ.setdefault("MSWEA_SILENT_STARTUP", "1")

import logging

from Iris_cli import __version__, __release_date__
from Iris_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from Iris_cli.config import get_env_path, get_Iris_home
    from Iris_cli.auth import get_auth_status
    ...
    # Check for Metaxis Portal OAuth credentials
    auth_file = get_Iris_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'Iris sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Iris isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  Iris setup")
        print()
        try:
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'Iris setup' at any time to configure.")
        sys.exit(1)
...
def cmd_gateway(args):
    ...
    from Iris_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from Iris_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Iris?")
        print()
    ...
    session_dir = Path.home() / ".Iris" / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: Iris gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  Iris gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Iris Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  Iris gateway")
        ...
        print()
        print("  Or install as a service: Iris gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'Iris whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from Iris_cli.setup import run_setup_wizard
    run_setup_wizard(args)


def cmd_model(args):
    """Select default model — starts with provider selection, then model picker."""
    from Iris_cli.auth import (
        ...
        resolve_Metaxis_runtime_credentials, fetch_Metaxis_models, AuthError, format_auth_error,
        _login_Metaxis,
    )
    from Iris_cli.config import load_config, save_config, get_env_value, save_env_value
    ...
    effective_provider = (
        os.getenv("Iris_INFERENCE_PROVIDER")
        or config_provider
        or "auto"
    )
    try:
    ...
    providers = [
        ...
        ("Metaxis", "Metaxis Portal (Metaxis Research subscription)"),
        ("openai-codex", "OpenAI Codex"),
    ...
    elif selected_provider == "Metaxis":
        _model_flow_Metaxis(config, current_model)
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from Iris_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from Iris_cli.config import get_env_value, save_env_value
    ...
    from Iris_cli.models import model_ids
...
def _model_flow_Metaxis(config, current_model=""):
    """Metaxis Portal provider: ensure logged in, then pick model."""
    from Iris_cli.auth import (
        get_provider_auth_state, _prompt_model_selection, _save_model_choice,
        _update_config_for_provider, resolve_Metaxis_runtime_credentials,
        fetch_Metaxis_models, AuthError, format_auth_error,
        _login_Metaxis, PROVIDER_REGISTRY,
    )
    from Iris_cli.config import get_env_value, save_env_value
    import argparse

    state = get_provider_auth_state("Metaxis")
    if not state or not state.get("access_token"):
        print("Not logged into Metaxis Portal. Starting login...")
        print()
        try:
            ...
            _login_Metaxis(mock_args, PROVIDER_REGISTRY["Metaxis"])
        except SystemExit:
            ...
            return
        # login_Metaxis already handles model selection + config update
        return

    # Already logged in — fetch models and select
    print("Fetching models from Metaxis Portal...")
    try:
        creds = resolve_Metaxis_runtime_credentials(min_key_ttl_seconds=5 * 60)
        model_ids = fetch_Metaxis_models(
        ...
        if relogin:
            ...
            print("Re-authenticating with Metaxis Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None, inference_url=None, client_id=None,
                    scope=None, no_browser=False, timeout=15.0,
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate Metaxis as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("Metaxis", inference_url)
        # Clear any custom endpoint that might conflict
        if get_env_value("OPENAI_BASE_URL"):
            save_env_value("OPENAI_BASE_URL", "")
            save_env_value("OPENAI_API_KEY", "")
        print(f"Default model set to: {selected} (via Metaxis Portal)")
    else:
        print("No change.")
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from Iris_cli.codex_models import get_codex_model_ids
    from Iris_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    try:
        from Iris_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from Iris_cli.auth import _save_model_choice, deactivate_provider
    from Iris_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        print("Endpoint saved. Use `/model` in chat or `Iris model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from Iris_cli.auth import _save_model_choice, deactivate_provider
    from Iris_cli.config import save_env_value, load_config, save_config
    from Iris_cli.models import fetch_api_models
...
def cmd_login(args):
    """Authenticate Iris CLI with a provider."""
    from Iris_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from Iris_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from Iris_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from Iris_cli.cron import cron_command
    cron_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from Iris_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from Iris_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Iris Agent v{__version__} ({__release_date__})")
...
def cmd_uninstall(args):
    """Uninstall Iris Agent."""
    from Iris_cli.uninstall import run_uninstall
    run_uninstall(args)


def _update_via_zip(args):
    """Update Iris Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/Metaxis Research/iris-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="Iris-update-")
        zip_path = os.path.join(tmp_dir, f"iris-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to iris-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"iris-agent-{branch}")
...
def cmd_update(args):
    """Update Iris Agent to the latest version."""
    import subprocess
    import shutil
    
    print("⚕ Updating Iris Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/Metaxis Research/iris-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        if needs_migration:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'Iris config migrate' later to configure.")
        else:
        ...
        try:
            check = subprocess.run(
                ["systemctl", "--user", "is-active", "Iris-gateway"],
            ...
            if check.stdout.strip() == "active":
                ...
                restart = subprocess.run(
                    ["systemctl", "--user", "restart", "Iris-gateway"],
                ...
                if restart.returncode == 0:
                    ...
                    print("  Try manually: Iris gateway restart")
        except (FileNotFoundError, subprocess.TimeoutExpired):
            pass  # No systemd (macOS, WSL1, etc.) — skip silently
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  Iris model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``Iris -c Pokemon Agent Dev`` without quoting the
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="Iris",
        description="Iris Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    Iris                        Start interactive chat
    Iris chat -q "Hello"        Single query mode
    Iris -c                     Resume the most recent session
    Iris -c "my project"        Resume a session by name (latest in lineage)
    Iris --resume <session_id>  Resume a specific session by ID
    Iris setup                  Run setup wizard
    Iris logout                 Clear stored authentication
    Iris model                  Select default model
    Iris config                 View configuration
    Iris config edit            Edit config in $EDITOR
    Iris config set model gpt-4 Set a config value
    Iris gateway                Run messaging gateway
    Iris -w                     Start in isolated git worktree
    Iris gateway install        Install as system service
    Iris sessions list          List past sessions
    Iris sessions browse        Interactive session picker
    Iris sessions rename ID T   Rename/title a session
    Iris update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Iris Agent"
    )
    ...
    chat_parser.add_argument(
        "--provider",
        choices=["auto", "openrouter", "Metaxis", "openai-codex", "zai", "kimi-coding", "minimax", "minimax-cn"],
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Iris Agent with an interactive wizard. "
                    "Run a specific section: Iris setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Iris CLI"
    )
    login_parser.add_argument(
        "--provider",
        choices=["Metaxis", "openai-codex"],
        default=None,
        help="Provider to authenticate with (default: Metaxis)"
    )
    login_parser.add_argument(
        "--portal-url",
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: Iris-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Iris Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Iris Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Iris Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from Iris_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            from Iris_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from Iris_cli.skills_hub import skills_command
            skills_command(args)
    ...
    def cmd_tools(args):
        from Iris_cli.tools_config import tools_command
        tools_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch Iris --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            Iris_bin = shutil.which("Iris")
            if Iris_bin:
                os.execvp(Iris_bin, ["Iris", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "Iris_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Iris"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Iris",
    ...
    def cmd_claw(args):
        from Iris_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Iris Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Iris Agent",
        description="Remove Iris Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    # e.g. ``Iris -c Pokemon Agent Dev`` → ``Iris -c 'Pokemon Agent Dev'``
