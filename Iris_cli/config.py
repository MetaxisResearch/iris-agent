"""
Configuration management for Iris Agent.

Config files are stored in ~/.Iris/ for easy access:
- ~/.Iris/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.Iris/.env         - API keys and secrets

This module provides:
- Iris config          - Show current configuration
- Iris config edit     - Open config in editor
- Iris config set      - Set a specific value
- Iris config wizard   - Re-run setup wizard
"""
...
def get_Iris_home() -> Path:
    """Get the Iris home directory (~/.Iris)."""
    return Path(os.getenv("Iris_HOME", Path.home() / ".Iris"))

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_Iris_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_Iris_home() / ".env"
...
def ensure_Iris_home():
    """Ensure ~/.Iris directory structure exists with secure permissions."""
    home = get_Iris_home()
...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | Metaxis | codex | custom
    ...
    # configured providers (OpenRouter, Metaxis, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # This section is only needed for Iris-specific overrides; everything else
...
# selection step (Metaxis Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "Metaxis_BASE_URL": {
        "description": "Metaxis Portal base URL override",
        "prompt": "Metaxis Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    # Iris_TOOL_PROGRESS and Iris_TOOL_PROGRESS_MODE are deprecated —
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("Iris_TOOL_PROGRESS")
            old_mode = get_env_value("Iris_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from Iris_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from Iris_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("Iris_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from Iris_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: Iris config set KEY VALUE")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.Iris/config.yaml."""
    import copy
...
#   openai-codex (OAuth — Iris login) — OpenAI Codex
#   Metaxis         (OAuth — Iris login) — Metaxis Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.Iris/config.yaml."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.Iris/.env."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.Iris/.env."""
    ensure_Iris_home()
    env_path = get_env_path()
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.Iris/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Iris Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  Iris config edit     # Edit config file", Colors.DIM))
    print(color("  Iris config set KEY VALUE", Colors.DIM))
    print(color("  Iris setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or not value:
            print("Usage: Iris config set KEY VALUE")
            print()
            print("Examples:")
            print("  Iris config set model anthropic/claude-sonnet-4")
            print("  Iris config set terminal.backend docker")
            print("  Iris config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print(f"    Run 'Iris config migrate' to add them")
        ...
        print("  Iris config           Show current configuration")
        print("  Iris config edit      Open config in editor")
        print("  Iris config set K V   Set a config value")
        print("  Iris config check     Check for missing/outdated config")
        print("  Iris config migrate   Update config with new options")
        print("  Iris config path      Show config file path")
        print("  Iris config env-path  Show .env file path")
        sys.exit(1)
