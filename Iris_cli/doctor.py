"""
Doctor command for Iris CLI.

Diagnoses issues with Iris Agent setup.
"""
...
from Iris_cli.config import get_project_root, get_Iris_home, get_env_path

PROJECT_ROOT = get_project_root()
Iris_HOME = get_Iris_home()

# Load environment variables from ~/.Iris/.env so API key checks work
...
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(Iris_HOME))
os.environ.setdefault("MSWEA_SILENT_STARTUP", "1")

from Iris_cli.colors import Colors, color
from Iris_constants import OPENROUTER_MODELS_URL
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.Iris/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Iris Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.Iris/.env (primary location for user config)
    env_path = Iris_HOME / '.env'
    if env_path.exists():
        check_ok("~/.Iris/.env file exists")
        ...
        if _has_provider_env_config(content):
            ...
            check_warn("No API key found in ~/.Iris/.env")
            issues.append("Run 'Iris setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            check_fail("~/.Iris/.env file missing")
            if should_fix:
                env_path.parent.mkdir(parents=True, exist_ok=True)
                env_path.touch()
                check_ok("Created empty ~/.Iris/.env")
                check_info("Run 'Iris setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'Iris setup' to create one")
                issues.append("Run 'Iris setup' to create .env")
    
    # Check ~/.Iris/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = Iris_HOME / 'config.yaml'
    if config_path.exists():
        check_ok("~/.Iris/config.yaml exists")
    else:
        ...
        if fallback_config.exists():
            ...
            if should_fix and example_config.exists():
                ...
                check_ok("Created ~/.Iris/config.yaml from cli-config.yaml.example")
                fixed_count += 1
            elif should_fix:
                check_warn("config.yaml not found and no example to copy from")
                manual_issues.append("Create ~/.Iris/config.yaml manually")
            else:
    ...
    try:
        from Iris_cli.auth import get_Metaxis_auth_status, get_codex_auth_status

        Metaxis_status = get_Metaxis_auth_status()
        if Metaxis_status.get("logged_in"):
            check_ok("Metaxis Portal auth", "(logged in)")
        else:
            check_warn("Metaxis Portal auth", "(not logged in)")
    ...
    Iris_home = Iris_HOME
    if Iris_home.exists():
        check_ok("~/.Iris directory exists")
    else:
        if should_fix:
            Iris_home.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.Iris directory")
            fixed_count += 1
        else:
            check_warn("~/.Iris not found", "(will be created on first use)")
    
    # Check expected subdirectories
    expected_subdirs = ["cron", "sessions", "logs", "skills", "memories"]
    for subdir_name in expected_subdirs:
        subdir_path = Iris_home / subdir_name
        if subdir_path.exists():
            check_ok(f"~/.Iris/{subdir_name}/ exists")
        else:
            if should_fix:
                subdir_path.mkdir(parents=True, exist_ok=True)
                check_ok(f"Created ~/.Iris/{subdir_name}/")
                fixed_count += 1
            else:
                check_warn(f"~/.Iris/{subdir_name}/ not found", "(will be created on first use)")
    
    # Check for SOUL.md persona file
    soul_path = Iris_home / "SOUL.md"
    if soul_path.exists():
        content = soul_path.read_text(encoding="utf-8").strip()
        # Check if it's just the template comments (no real content)
        lines = [l for l in content.splitlines() if l.strip() and not l.strip().startswith(("<!--", "-->", "#"))]
        if lines:
            check_ok("~/.Iris/SOUL.md exists (persona configured)")
        else:
            check_info("~/.Iris/SOUL.md exists but is empty — edit it to customize personality")
    else:
        check_warn("~/.Iris/SOUL.md not found", "(create it to give Iris a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Iris Agent Persona\n\n"
                "<!-- Edit this file to customize how Iris communicates. -->\n\n"
                "You are Iris, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            check_ok("Created ~/.Iris/SOUL.md with basic template")
            fixed_count += 1
    
    # Check memory directory
    memories_dir = Iris_home / "memories"
    if memories_dir.exists():
        check_ok("~/.Iris/memories/ directory exists")
        ...
        check_warn("~/.Iris/memories/ not found", "(will be created on first use)")
        if should_fix:
            memories_dir.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.Iris/memories/")
            fixed_count += 1
    
    # Check SQLite session store
    state_db_path = Iris_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            conn.close()
            check_ok(f"~/.Iris/state.db exists ({count} sessions)")
        except Exception as e:
            check_warn(f"~/.Iris/state.db exists but has issues: {e}")
    else:
        check_info("~/.Iris/state.db not created yet (will be created on first session)")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/Metaxis Research/iris-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'Iris setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: Iris skills list)")

    from Iris_cli.config import get_env_value
    github_token = get_env_value("GITHUB_TOKEN") or get_env_value("GH_TOKEN")
    if github_token:
        check_ok("GitHub token configured (authenticated API access)")
    else:
        check_warn("No GITHUB_TOKEN", "(60 req/hr rate limit — set in ~/.Iris/.env for better rates)")
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'Iris doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
