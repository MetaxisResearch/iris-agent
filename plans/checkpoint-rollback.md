...
5. **User-facing rollback** — `/rollback` slash command (CLI + gateway) to list and restore checkpoints. Also `Iris rollback` CLI subcommand.
...
~/.Iris/checkpoints/
  {sha256(abs_dir)[:16]}/       # Shadow git repo per working directory
    ...
    Iris_WORKDIR              # Original dir path (for display)
...
Add to `DEFAULT_CONFIG` in `Iris_cli/config.py`:
...
**CLI subcommand** — `Iris rollback` (optional, lower priority).
...
| `Iris_cli/config.py` | Add `checkpoints` + `checkpoint_max_snapshots` to DEFAULT_CONFIG |
...
3. `Iris_cli/config.py` — config keys
