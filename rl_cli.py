...
if tinker_atropos_dir.exists():
    ...
    os.environ['Iris_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to iris-agent directory if submodule not found
...
def list_environments_sync():
    """List available environments (synchroMetaxis wrapper)."""
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.Iris/config.yaml if not provided)
    ...
    """
    # Load config from ~/.Iris/config.yaml
    config = load_Iris_config()
