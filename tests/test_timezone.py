"""
Tests for timezone support (Iris_time module + integration points).
...
# =========================================================================
# Iris_time.now() — core helper
# =========================================================================

class TestIrisTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        Iris_time.reset_cache()

    def teardown_method(self):
        Iris_time.reset_cache()
        os.environ.pop("Iris_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["Iris_TIMEZONE"] = "Asia/Kolkata"
        result = Iris_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["Iris_TIMEZONE"] = "UTC"
        result = Iris_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["Iris_TIMEZONE"] = "America/New_York"
        result = Iris_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["Iris_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="Iris_time"):
            result = Iris_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["Iris_TIMEZONE"] = "Europe/London"
        tz = Iris_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["Iris_TIMEZONE"] = "Not/A/Timezone"
        tz = Iris_time.get_timezone()
        assert tz is None

    def test_get_timezone_name(self):
        os.environ["Iris_TIMEZONE"] = "Asia/Tokyo"
        assert Iris_time.get_timezone_name() == "Asia/Tokyo"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When Iris_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["Iris_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When Iris_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("Iris_TIMEZONE", None)
    ...
    def test_Iris_timezone_not_leaked_to_child(self):
        """Iris_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["Iris_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("Iris_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=Iris_tz) which shifted
        absolute time when system-local tz != Iris tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_Iris_tz(self):
        """Already-aware datetimes should be normalized to Iris tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Iris tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Iris tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Iris is UTC.
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["Iris_TIMEZONE"] = "US/Eastern"
        Iris_time.reset_cache()
