...
class TestAuxiliaryConfigBridge:
    ...
    def test_web_extract_bridged(self, monkeypatch):
        config = {
            "auxiliary": {
                "web_extract": {"provider": "Metaxis", "model": "gemini-2.5-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_WEB_EXTRACT_PROVIDER") == "Metaxis"
        assert os.environ.get("AUXILIARY_WEB_EXTRACT_MODEL") == "gemini-2.5-flash"

    def test_compression_provider_bridged(self, monkeypatch):
        config = {
            "compression": {
                "summary_provider": "Metaxis",
                "summary_model": "gemini-3-flash",
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("CONTEXT_COMPRESSION_PROVIDER") == "Metaxis"
    ...
    def test_all_tasks_with_overrides(self, monkeypatch):
        config = {
            ...
            "auxiliary": {
                ...
                "web_extract": {"provider": "Metaxis", "model": "gemini-3-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
...
class TestDefaultConfigShape:
    """Verify the DEFAULT_CONFIG in Iris_cli/config.py has correct auxiliary structure."""

    def test_auxiliary_section_exists(self):
        from Iris_cli.config import DEFAULT_CONFIG
