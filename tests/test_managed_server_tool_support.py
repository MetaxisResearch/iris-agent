...
4. iris-agent's tool_call_parsers are compatible with ManagedServer's expectations

These tests verify the contract between iris-agent's environments/ code
...
class TestManagedServerAPI:
    """Test that ManagedServer's API matches what iris-agent expects."""
...
class TestParserCompatibility:
    """Test that iris-agent's parsers match ManagedServer's expectations."""
...
class TestBaseEnvCompatibility:
    """Test that Iris_base_env.py's managed_server() call matches the API."""

    def test_Iris_base_env_managed_server_call_pattern(self):
        """
        Verify that Iris_base_env.py passes tool_call_parser to managed_server().
        This is a source-level check — the actual managed_server() call must match.
        """
        import ast

        base_env_path = Path(__file__).parent.parent / "environments" / "Iris_base_env.py"
        ...
        assert found_tool_call_parser_kwarg, (
            "Iris_base_env.py should pass tool_call_parser= to managed_server()"
        )

    def test_Iris_base_env_uses_get_parser(self):
        """Verify Iris_base_env imports and uses get_parser from tool_call_parsers."""
        base_env_path = Path(__file__).parent.parent / "environments" / "Iris_base_env.py"
        source = base_env_path.read_text()
