...
class TestRunConversation:
    ...
    def test_Metaxis_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "Metaxis"
        ...
        with (
            ...
            patch.object(agent, "_try_refresh_Metaxis_client_credentials", side_effect=_fake_refresh),
        ):
            result = agent.run_conversation("hello")
...
class TestMetaxisCredentialRefresh:
    """Verify Metaxis credential refresh rebuilds the runtime client."""

    def test_try_refresh_Metaxis_client_credentials_rebuilds_client(self, agent, monkeypatch):
        agent.provider = "Metaxis"
        agent.api_mode = "chat_completions"
        ...
        monkeypatch.setattr("Iris_cli.auth.resolve_Metaxis_runtime_credentials", _fake_resolve)

        agent.client = _ExistingClient()
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_Metaxis_client_credentials(force=True)

        assert ok is True
        assert closed["value"] is True
        assert captured["force_mint"] is True
        assert rebuilt["kwargs"]["api_key"] == "new-Metaxis-key"
        assert rebuilt["kwargs"]["base_url"] == "https://inference-api.Metaxis Research.com/v1"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Iris Agent" in agent._cached_system_prompt
