...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("Iris_cli.config.get_config_path", return_value=config_path), \
             patch("Iris_cli.config.ensure_Iris_home"):
            from Iris_cli.config import save_config
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("Iris_cli.config.get_env_path", return_value=env_path), \
             patch("Iris_cli.config.ensure_Iris_home"):
    ...
    def test_ensure_Iris_home_sets_0700(self):
        home = Path(self.tmpdir) / ".Iris"
        with patch("Iris_cli.config.get_Iris_home", return_value=home):
            from Iris_cli.config import ensure_Iris_home
            ensure_Iris_home()
