...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "Metaxis_INFERENCE_BASE_URL",
...
class TestReadCodexAccessToken:
    def test_valid_auth_store(self, tmp_path, monkeypatch):
        ...
        (Iris_home / "auth.json").write_text(json.dumps({
            "version": 1,
            "providers": {
                "openai-codex": {
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        Iris_home = tmp_path / "Iris"
        Iris_home.mkdir(parents=True, exist_ok=True)
        (Iris_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
        monkeypatch.setenv("Iris_HOME", str(Iris_home))
        result = _read_codex_access_token()
...
class TestGetTextAuxiliaryClient:
    ...
    def test_Metaxis_takes_priority_over_codex(self, monkeypatch, codex_auth_dir):
        with patch("agent.auxiliary_client._read_Metaxis_auth") as mock_Metaxis, \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_Metaxis.return_value = {"access_token": "Metaxis-tok"}
            client, model = get_text_auxiliary_client()
    ...
    def test_custom_endpoint_over_codex(self, monkeypatch, codex_auth_dir):
        ...
        with patch("agent.auxiliary_client._read_Metaxis_auth", return_value=None), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            client, model = get_text_auxiliary_client()
...
class TestVisionClientFallback:
    """Vision client auto mode only tries OpenRouter + Metaxis (multimodal-capable)."""
    ...
    def test_vision_auto_falls_back_to_custom_endpoint(self, monkeypatch):
        ...
        When no OpenRouter/Metaxis/Codex is available, try the custom endpoint.
        """
        monkeypatch.setenv("OPENAI_BASE_URL", "http://localhost:1234/v1")
    ...
    def test_vision_uses_Metaxis_when_available(self, monkeypatch):
        with patch("agent.auxiliary_client._read_Metaxis_auth") as mock_Metaxis, \
             patch("agent.auxiliary_client.OpenAI"):
            mock_Metaxis.return_value = {"access_token": "Metaxis-tok"}
            client, model = get_vision_auxiliary_client()
...
class TestGetAuxiliaryProvider:
    ...
    def test_context_prefix_fallback(self, monkeypatch):
        monkeypatch.setenv("CONTEXT_COMPRESSION_PROVIDER", "Metaxis")
        assert _get_auxiliary_provider("compression") == "Metaxis"
...
class TestResolveForcedProvider:
    ...
    def test_forced_Metaxis(self, monkeypatch):
        with patch("agent.auxiliary_client._read_Metaxis_auth") as mock_Metaxis, \
             patch("agent.auxiliary_client.OpenAI"):
            mock_Metaxis.return_value = {"access_token": "Metaxis-tok"}
            client, model = _resolve_forced_provider("Metaxis")
        assert model == "gemini-3-flash"
        assert client is not None

    def test_forced_Metaxis_not_configured(self, monkeypatch):
        with patch("agent.auxiliary_client._read_Metaxis_auth", return_value=None):
            client, model = _resolve_forced_provider("Metaxis")
        assert client is None
        assert model is None
    ...
    def test_forced_main_skips_openrouter_Metaxis(self, monkeypatch):
...
class TestTaskSpecificOverrides:
    ...
    def test_text_with_vision_provider_override(self, monkeypatch):
        ...
        monkeypatch.setenv("AUXILIARY_VISION_PROVIDER", "Metaxis")
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client.OpenAI"):
            client, model = get_text_auxiliary_client()  # no task → auto
        assert model == "google/gemini-3-flash-preview"  # OpenRouter, not Metaxis
    ...
    def test_compression_task_reads_context_prefix(self, monkeypatch):
        ...
        assert model == "gemini-3-flash"  # forced to Metaxis, not OpenRouter
