...
class TestTryActivateFallback:
    ...
    def test_activates_Metaxis_fallback(self):
        """Metaxis Portal fallback should use OAuth credentials and chat_completions mode."""
        agent = _make_agent(
            fallback_model={"provider": "Metaxis", "model": "Metaxis-Iris-3"},
        )
        mock_client = _mock_resolve(
            api_key="Metaxis-agent-key-abc",
            base_url="https://inference-api.Metaxis Research.com/v1",
        )
        with patch(
            "agent.auxiliary_client.resolve_provider_client",
            return_value=(mock_client, "Metaxis-Iris-3"),
        ):
            result = agent._try_activate_fallback()
            assert result is True
            assert agent.model == "Metaxis-Iris-3"
            assert agent.provider == "Metaxis"
            assert agent.api_mode == "chat_completions"
            assert agent.client is mock_client

    def test_Metaxis_fallback_fails_gracefully_without_login(self):
        """Metaxis fallback should return False if not logged in."""
        agent = _make_agent(
            fallback_model={"provider": "Metaxis", "model": "Metaxis-Iris-3"},
        )
        with patch(
