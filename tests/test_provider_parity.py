...
class TestBuildApiKwargsMetaxisPortal:
    def test_includes_Metaxis_product_tags(self, monkeypatch):
        agent = _make_agent(monkeypatch, "Metaxis", base_url="https://inference-api.Metaxis Research.com/v1")
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == ["product=iris-agent"]
...
# ── Chat completions response handling (OpenRouter/Metaxis) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_Metaxis_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import get_text_auxiliary_client
        with patch("agent.auxiliary_client._read_Metaxis_auth", return_value={"access_token": "Metaxis-tok"}), \
             patch("agent.auxiliary_client.OpenAI") as mock:
            client, model = get_text_auxiliary_client()
        assert model == "gemini-3-flash"

    def test_custom_endpoint_when_no_Metaxis(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
