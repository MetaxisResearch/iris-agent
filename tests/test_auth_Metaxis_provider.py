"""Regression tests for Metaxis OAuth refresh + agent-key mint interactions."""
...
from Iris_cli.auth import AuthError, get_provider_auth_state, resolve_Metaxis_runtime_credentials


def _setup_Metaxis_auth(
    Iris_home: Path,
    *,
    ...
    (Iris_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    _setup_Metaxis_auth(Iris_home, refresh_token="refresh-old")
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        ...
        if mint_calls["count"] == 1:
            raise AuthError("credits exhausted", provider="Metaxis", code="insufficient_credits")
        return _mint_payload(api_key="agent-key-2")

    monkeypatch.setattr("Iris_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("Iris_cli.auth._mint_agent_key", _fake_mint_agent_key)

    with pytest.raises(AuthError) as exc:
        resolve_Metaxis_runtime_credentials(min_key_ttl_seconds=300)
    assert exc.value.code == "insufficient_credits"

    state_after_failure = get_provider_auth_state("Metaxis")
    assert state_after_failure is not None
    assert state_after_failure["refresh_token"] == "refresh-1"
    assert state_after_failure["access_token"] == "access-1"

    creds = resolve_Metaxis_runtime_credentials(min_key_ttl_seconds=300)
...
def test_mint_retry_uses_latest_rotated_refresh_token(tmp_path, monkeypatch):
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        mint_calls["count"] += 1
        if mint_calls["count"] == 1:
            raise AuthError("stale access token", provider="Metaxis", code="invalid_token")
