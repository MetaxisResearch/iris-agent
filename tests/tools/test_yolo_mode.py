"""Tests for --yolo (Iris_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When Iris_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("Iris_INTERACTIVE", "1")
        monkeypatch.setenv("Iris_SESSION_KEY", "test-session")
        monkeypatch.delenv("Iris_YOLO_MODE", raising=False)
        monkeypatch.delenv("Iris_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("Iris_EXEC_ASK", raising=False)
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With Iris_YOLO_MODE, dangerous commands are auto-approved."""
        monkeypatch.setenv("Iris_YOLO_MODE", "1")
        monkeypatch.setenv("Iris_INTERACTIVE", "1")
        monkeypatch.setenv("Iris_SESSION_KEY", "test-session")
    ...
    def test_yolo_mode_not_set_by_default(self):
        """Iris_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("Iris_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for Iris_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("Iris_YOLO_MODE", "")
        monkeypatch.setenv("Iris_INTERACTIVE", "1")
        monkeypatch.setenv("Iris_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("Iris_YOLO_MODE") returns ""
