...
class TestDelegationCredentialResolution(unittest.TestCase):
    ...
    @patch("Iris_cli.runtime_provider.resolve_runtime_provider")
    ...
    @patch("Iris_cli.runtime_provider.resolve_runtime_provider")
    def test_Metaxis_provider_resolves_Metaxis_credentials(self, mock_resolve):
        """Metaxis provider resolves Metaxis Portal base_url and api_key."""
        mock_resolve.return_value = {
            "provider": "Metaxis",
            "base_url": "https://inference-api.Metaxis Research.com/v1",
            "api_key": "Metaxis-agent-key-xyz",
            "api_mode": "chat_completions",
        }
        parent = _make_mock_parent(depth=0)
        cfg = {"model": "Iris-3-llama-3.1-8b", "provider": "Metaxis"}
        creds = _resolve_delegation_credentials(cfg, parent)
        self.assertEqual(creds["provider"], "Metaxis")
        self.assertEqual(creds["base_url"], "https://inference-api.Metaxis Research.com/v1")
        self.assertEqual(creds["api_key"], "Metaxis-agent-key-xyz")
        mock_resolve.assert_called_once_with(requested="Metaxis")
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_cross_provider_delegation(self, mock_creds, mock_cfg):
        """Parent on Metaxis, subagent on OpenRouter — full credential switch."""
        mock_cfg.return_value = {
            "max_iterations": 45,
            "model": "google/gemini-3-flash-preview",
            "provider": "openrouter",
        ...
        parent = _make_mock_parent(depth=0)
        parent.provider = "Metaxis"
        parent.base_url = "https://inference-api.Metaxis Research.com/v1"
        parent.api_key = "Metaxis-key-abc"
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            # Child should use OpenRouter, NOT Metaxis
            self.assertEqual(kwargs["provider"], "openrouter")
            self.assertEqual(kwargs["base_url"], "https://openrouter.ai/api/v1")
