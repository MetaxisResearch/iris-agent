"""Tests for Iris_cli configuration management."""
...
class TestGetIrisHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("Iris_HOME", None)
            home = get_Iris_home()
            assert home == Path.home() / ".Iris"

    def test_env_override(self):
        with patch.dict(os.environ, {"Iris_HOME": "/custom/path"}):
            home = get_Iris_home()
            assert home == Path("/custom/path")


class TestEnsureIrisHome:
