"""Tests for provider-aware `/model` validation in Iris_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("Iris_cli.models.fetch_api_models", return_value=api_models):
...
class TestParseModelInput:
    ...
    def test_provider_colon_model_switches_provider(self):
        provider, model = parse_model_input("openrouter:anthropic/claude-sonnet-4.5", "Metaxis")
        assert provider == "openrouter"
        assert model == "anthropic/claude-sonnet-4.5"
    ...
    def test_Metaxis_provider_switch(self):
        provider, model = parse_model_input("Metaxis:Iris-3", "openrouter")
        assert provider == "Metaxis"
        assert model == "Iris-3"

    def test_empty_model_after_colon_keeps_current(self):
        provider, model = parse_model_input("openrouter:", "Metaxis")
        assert provider == "Metaxis"
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("Iris_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
