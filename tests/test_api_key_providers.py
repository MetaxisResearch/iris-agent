...
class TestProviderRegistry:
    ...
    def test_oauth_providers_unchanged(self):
        ...
        assert "Metaxis" in PROVIDER_REGISTRY
        assert PROVIDER_REGISTRY["Metaxis"].auth_type == "oauth_device_code"
...
class TestApiKeyProviderStatus:
    ...
    def test_non_api_key_provider(self):
        status = get_api_key_provider_status("Metaxis")
        assert status["configured"] is False
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_invalid_provider_raises(self):
        with pytest.raises(AuthError):
            resolve_api_key_provider_credentials("Metaxis")
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from Iris_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        Iris_home = tmp_path / ".Iris"
        Iris_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: Iris_home / ".env")
        monkeypatch.setattr(config_module, "get_Iris_home", lambda: Iris_home)
        from Iris_cli.main import _has_any_provider_configured
