...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Iris venv.
version: 1.0.0
author: Metaxis Research
license: MIT
metadata:
  Iris:
...
```bash
GSETUP="python ~/.Iris/skills/productivity/google-workspace/scripts/setup.py"
```
...
- Token is stored at `~/.Iris/google_token.json` and auto-refreshes.
...
```bash
GAPI="python ~/.Iris/skills/productivity/google-workspace/scripts/google_api.py"
```
