...
from Iris_constants import OPENROUTER_BASE_URL, OPENROUTER_MODELS_URL
...
class _SafeWriter:
    ...
    When iris-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        # Persistent error log -- always writes WARNING+ to ~/.Iris/logs/errors.log
        # so tool failures, API errors, etc. are inspectable after the fact.
        from agent.redact import RedactingFormatter
        _error_log_dir = Path.home() / ".Iris" / "logs"
        ...
        # Session logs go into ~/.Iris/sessions/ alongside gateway sessions
        Iris_home = Path(os.getenv("Iris_HOME", Path.home() / ".Iris"))
        self.logs_dir = Iris_home / "sessions"
        ...
        if not skip_memory:
            try:
                from Iris_cli.config import load_config as _load_mem_config
        ...
        try:
            from Iris_cli.config import load_config as _load_skills_config
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if os.getenv("Iris_DUMP_REQUEST_STDOUT", "").strip().lower() in {"1", "true", "yes", "on"}:
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        from Iris_time import now as _Iris_now
        now = _Iris_now()
    ...
    def _try_refresh_Metaxis_client_credentials(self, *, force: bool = True) -> bool:
        if self.api_mode != "chat_completions" or self.provider != "Metaxis":
            return False

        try:
            from Iris_cli.auth import resolve_Metaxis_runtime_credentials

            creds = resolve_Metaxis_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("Iris_Metaxis_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("Iris_Metaxis_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
            logger.debug("Metaxis credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # Metaxis requests should not inherit OpenRouter-only attribution headers.
        ...
        try:
            ...
            logger.warning("Failed to rebuild OpenAI client after Metaxis refresh: %s", exc)
            return False
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        # TODO: Metaxis Portal will add transparent proxy support — re-enable
        # for _is_Metaxis when their backend is updated.
        if provider_preferences and _is_openrouter:
            extra_body["provider"] = provider_preferences
        _is_Metaxis = "Metaxis Research" in self.base_url.lower()

        _is_mistral = "api.mistral.ai" in self.base_url.lower()
        if (_is_openrouter or _is_Metaxis) and not _is_mistral:
            if self.reasoning_config is not None:
                rc = dict(self.reasoning_config)
                # Metaxis Portal requires reasoning enabled — don't send
                # enabled=false to it (would cause 400).
                if _is_Metaxis and rc.get("enabled") is False:
                    pass  # omit reasoning entirely for Metaxis when disabled
                else:
        ...
        # Metaxis Portal product attribution
        if _is_Metaxis:
            extra_body["tags"] = ["product=iris-agent"]
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            _is_openrouter = "openrouter" in self.base_url.lower()
            _is_Metaxis = "Metaxis Research" in self.base_url.lower()
            if _is_openrouter or _is_Metaxis:
                if self.reasoning_config is not None:
            ...
            if _is_Metaxis:
                summary_extra_body["tags"] = ["product=iris-agent"]
    ...
    def run_conversation(
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            Metaxis_auth_retry_attempted = False
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if os.getenv("Iris_DUMP_REQUESTS", "").strip().lower() in {"1", "true", "yes", "on"}:
                    ...
                    if (
                        self.api_mode == "chat_completions"
                        and self.provider == "Metaxis"
                        and status_code == 401
                        and not Metaxis_auth_retry_attempted
                    ):
                        Metaxis_auth_retry_attempted = True
                        if self._try_refresh_Metaxis_client_credentials(force=True):
                            print(f"{self.log_prefix}🔐 Metaxis agent key refreshed after 401. Retrying request...")
                            continue
                    ...
                    is_context_length_error = any(phrase in error_msg for phrase in [
                        ...
                        'request entity too large',  # OpenRouter/Metaxis 413 safety net
