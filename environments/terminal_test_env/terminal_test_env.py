...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Iris Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Iris Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Iris Agent",
    },
    {
...
class TerminalTestEnv(IrisAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/iris-agent/.env.
        """
        env_config = TerminalTestEnvConfig(
            ...
            tokenizer_name="Metaxis Research/q-30b-t-h45-e1",
