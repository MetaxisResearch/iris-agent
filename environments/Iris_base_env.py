"""
IrisAgentBaseEnv -- Abstract Base Environment for iris-agent + Atropos

Provides the Atropos integration plumbing that all iris-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via IrisAgentLoop
...
# Ensure the iris-agent repo root is on sys.path so that imports like
...
# Load API keys from iris-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, IrisAgentLoop
from environments.tool_context import ToolContext

# Import iris-agent toolset infrastructure
...
class IrisAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for iris-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of Iris toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: Iris, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class IrisAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for iris-agent Atropos integration.
    ...
    name: Optional[str] = "iris-agent"
    env_config_cls = IrisAgentEnvConfig

    def __init__(
        self,
        config: IrisAgentEnvConfig,
        ...
        # Set terminal environment variables so Iris tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY iris-agent tool (terminal, file, web,
