...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs IrisAgentLoop with terminal tool only
...
Requires: pip install iris-agent[yc-bench]
"""
...
class YCBenchEvalConfig(IrisAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends IrisAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(IrisAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The IrisAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs IrisAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our IrisAgentLoop.
